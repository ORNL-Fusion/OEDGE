...
*
      ...
      REAL(DP) :: H1RN, H1RN_REINIT_RESULT
...
cpb         WRITE(iunout,*) 
CPB     .     ' H1RN (RANMAR): INITIALIZED WITH DEFAULT SEED'
            ISEED1      = 12345
            ISEED2      = 98765
...
C     following entry is for reinitialization of eirene
      
      ENTRY H1RN_REINIT() RESULT (H1RN_REINIT_RESULT)
      FIRST = .TRUE.
c      flag = 'not init.'
      H1RN_REINIT_RESULT=0.D0
      return
      END
C ===== SOURCE: h1rnin.f
*
...
cpb   WRITE(iunout,610) IJ,KL,I,J,K,L
...
C ===== SOURCE: quadeq.f
      ...
      IER = 0
      IF ( ABS(A) < EPS10) THEN
        WRITE (iunout,*) ' A = 0 IN QUADEQ '
        IER = 1
      ELSE
        BH = B/(2.D0*A)
        ROOT = BH*BH - C/A
        IF ( ROOT < 0.D0 ) THEN
          WRITE (iunout,*) ' PROBLEM: SQRT(-X) IN QUADEQ '
          IER = 2
        ELSE
          SQ = SQRT(ROOT)
          T1 = -BH + SQ
