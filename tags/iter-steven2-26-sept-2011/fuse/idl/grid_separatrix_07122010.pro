...
; ======================================================================
;
FUNCTION grid_LoadWall, file, debug=debug
...
FUNCTION grid_AnalyseBoundary, b, wall, xpoint_zone, mode, debug=debug

  psi   = b.psi_raw
  psi_x = b.x
  psi_y = b.y

  psi_1st_separatrix = b.psi[b.null_i[1],b.null_j[1]]
  psi_2nd_separatrix = b.psi[b.null_i[2],b.null_j[2]]
  ...
  psi_val    = DOUBLE(psi_1st_separatrix) - 0.001
  psi_start  = psi_val
  psi_end    = psi_1st_separatrix - 3.0D
  psi_step   = -0.005D ; -0.5D
  psi_adjust = psi_step
  tol_adjust = 1.0D-5
  ...
  IF (0 EQ 1) THEN BEGIN
    ...
    ctr = grid_ExtractContour(psi, psi_x, psi_y, psi_1st_separatrix)
    PLOT,ctr.x,ctr.y,color=Truecolor('Green'),  $
         XRANGE=xrange,YRANGE=yrange,XSTYLE=1,YSTYLE=1,/NOERASE
    ctr = grid_ExtractContour(psi, psi_x, psi_y, psi_2nd_separatrix)
...
;  
  ...
  WHILE (the_search_continues) DO BEGIN
    ...
    IF (1 EQ 1 AND contour_n EQ 0) THEN BEGIN
      ...
      grid_SetActiveContour, contour_array, int_nlast, region, active_contour, store_forced,  $
                             ...
                             the_search_continues, contour_n
      the_search_continues = 1
      psi_start = psi_2nd_separatrix - 0.0001D
      psi_val = psi_start
      process_2nd = 2
      int_nlast[SOL] = 2
    ENDIF
    ...
    print,'psi_values: ',psi_start,psi_val,psi_end,process_2nd



    IF (process_2nd EQ 0 AND  $
        psi_adjust NE psi_step AND psi_val LT psi_2nd_separatrix) THEN  $
      process_2nd = 1
...
;      yrange = [-2.0,-1.0]
      ...
      XRANGE=xrange,YRANGE=yrange,/NOERASE,XSTYLE=1,YSTYLE=1
         ...
         PLOT,[boundary2_p1[0],boundary2_p2[0]],  $
              ...
              XRANGE=xrange,YRANGE=yrange,XSTYLE=1,YSTYLE=1,/NOERASE
print,'... nseg',nseg
;      if (iteration_count EQ 24) then stop
    ENDIF
...
;  
    FOR iseg = 0, nseg DO BEGIN
      ...
      IF (save_boundary1_p1[0] NE -999.0D0) THEN BEGIN
        IF (seg_active[iseg] EQ 1 AND psi_val LT psi_2nd_separatrix) THEN BEGIN
      ...
      WHILE (cont EQ 1 OR cont EQ 3) DO BEGIN
        ...
        print,'  boundary2:',FLOAT(boundary2_p1),FLOAT(boundary2_p2),FORMAT='(A,4F10.4)'
              print,'psi...',psi_val,psi_2nd_separatrix,psi_val GT psi_2nd_separatrix
        print,N_ELEMENTS(x)
        ...
        ELSE BEGIN
          IF (failure EQ 1 OR failure EQ 2) THEN BEGIN
            ...
            FOR i = 0, N_ELEMENTS(x)-2 DO BEGIN
              ...
              IF (status) THEN BREAK
            ENDFOR
            IF (NOT status) THEN cont = 2 ELSE BEGIN

              print,'checking...'
              print,'psi...',psi_val,psi_2nd_separatrix
              ...
              IF (     psi_val LT psi_2nd_separatrix AND  $
                  last_psi_val GT psi_2nd_separatrix) THEN BEGIN
...
; the boundary when inside the x-point again:
      ...
      ; Cycle to the next valid contour if processing outside the 2nd separatrix:
...
;                   XRANGE=xrange,YRANGE=yrange,XSTYLE=1,YSTYLE=1,/NOERASE
            ...
            IF ((psi_adjust GE 0.0D AND psi_adjust LT tol_adjust) OR  $
              ...
              if (contour_n EQ 400) then begin
                xrange = [ 3.0,9.0]
                yrange = [-6.0,6.0]
...
; ======================================================================
;
FUNCTION grid_Main, iter=iter, mast=mast
...
;      b = grid_ReadEQUFile('/home/ITER/lisgos/divimp/shots/iter/1514/Baseline2008-li0.70.x4.equ');,/CHANGE_SIGN)
      ...
      result = grid_AnalyseBoundary(b,wall,xpoint_zone,1,/debug)
