...
C
     ...
     .          TBCX, PLS, RATE_COEFF, SNGL_POLY, RCMIN, RCMAX,
     .          ELTHDUM, CTCHDUM,  
     .          FEELEI1, FEELPI1,
     .          FEHVDS1, FEHVPI3,
     .          ENERGY_RATE_COEFF, FTABPI3, CII, FEPLPI3, ERATE
...
!pb            TBPI = RATE_COEFF(KK,PLS,0._DP,.TRUE.,0,ERATE)*DIIN(IPLS,K)
!pb            SIGVPI(IRPI)=TBPI
            SIGVPI(IRPI)=FTABPI3(IRPI,K)
          END IF
...
! CALCULATE RATE-COEFFICIENT 
            ...
            EXPO = RATE_COEFF(KK,PLS,ELB,.FALSE.,0,ERATE) 
...
cdr     ELSE
cdr       ESIGPI(IRPI,4)=FEPLPI3(IRPI,K)
cdr     END IF
        CFLAG(4,1)=2
36    CONTINUE
C
...
CDR  SIGVCX(IRCX)=FTABCX3 : NOT READY
            ...
            TBCX = RATE_COEFF(KK,PLS,0._DP,.TRUE.,0,ERATE)*DIIN(IPLS,K)
            SIGVCX(IRCX)=TBCX
          END IF
...
C  MAXWELL
          ...
          ELSE
            ...
            TBEL = RATE_COEFF(KK,PLS,0._DP,.TRUE.,0,ERATE)*DIIN(IPLS,K)
            SIGVEL(IREL)=TBEL
          END IF
...
C  TEMPERATURE TOO LOW, USE: BEAM_ATOM - BEAM_DRIFT RATECOEFF.
            ...
            CEL=CROSS(ELAB,IREAC,IREL,'FPATHM EL1')
...
C  FIND SIGMA FROM AMJUEL DATA TABLES (BACHMANN ET AL.)
            CEL=CROSS(ELAB,IREAC,IREL,'FPATHM EL2')
          END IF
...
C  BEAM - BEAM RATE, IGNORE THERMAL ION ENERGY
          ...
          ELAB=LOG(VRELQ)+DEFEL(IREL)
          IREAC=MODCOL(5,1,IREL)
          CEL=CROSS(ELAB,IREAC,IREL,'FPATHM EL3')
          SIGVEL(IREL)=CEL*VREL*DENIO(IPLS)
        ELSE
          GOTO 995
        ENDIF
