...
C  RATE COEFFICIENT: (CM^3/S)
          DO J=1,NSBOX
            ...
            COU = RATE_COEFF(KK,TEINL(J),0._DP,.TRUE.,0,ERATE)
...
C  2.D) RATE COEFFICIENT(TE,NE)
     ...
     .        (IDEZ(ISCDE,5,5) == 3). AND. REACDAT(KREAD)%LRTCEW ) THEN
            IF (REACDAT(KREAD)%RTCEW%IFIT == 5) LHCOL=.TRUE.
          END IF
          DO J=1,NSBOX
            IF (LGVAC(J,NPLS+1)) CYCLE
            COU = RATE_COEFF(KK,TEINL(J),PLS(J),.FALSE.,1,ERATE)
            TB = COU + FCTKKL
