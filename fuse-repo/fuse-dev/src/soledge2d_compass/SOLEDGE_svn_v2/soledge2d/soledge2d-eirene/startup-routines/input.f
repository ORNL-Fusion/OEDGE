...
!pb      USE EIRMOD_CREFMOD
      ...
      USE EIRMOD_CUPD
      USE EIRMOD_PHOTON
      use styx2eirene, only : transfer_debug 
...
! CHECK GEOMETRY SWITCHES
     ...
     .                    ' FLAGS MAY BE .TRUE. '
       ...
       END IF
c Yannick (i8 for NRKNOT)
       READ (IUNIN,6686) NR1ST,NRSEP,NRPLG,NPPLG,NRKNOT,NCOOR 
...
625       IF (INDEX(ZEILE,'ON')+INDEX(ZEILE,'on').EQ.0) THEN
        ...
        ENDIF
      ENDIF
c yannick avoid consistency issues with format 6664
      READ (IUNIN,6664) (DATD(IATM),IATM=1,min(NATMI_IN,6))
620   READ (IUNIN,6664) (DMLD(IMOL),IMOL=1,min(NMOLI_IN,6))
      READ (IUNIN,6664) (DIOD(IION),IION=1,min(NIONI_IN,6))
      READ (IUNIN,6664) (DPLD(IPLS),IPLS=1,min(NPLSI_IN,6))
      IF (NPHOTI > 0)
...
713     READ (IUNIN,'(A72)') ZEILE
     ...
     .                    NLRAY(ISTRA)
        IREAD=0
c   Yannick format i6->i8
c        READ (IUNIN,6666) NPTS(ISTRA),NINITL(I),NEMODS(I),NAMODS(I),
c     .                    NPTSDEL(I),NRAYEN(ISTRA)

         READ (IUNIN,6668) NPTS(ISTRA),NINITL(I),NEMODS(I),NAMODS(I),
...
6666  FORMAT (12I6)
c Yannick i6-> i8 for number of particles
6668  FORMAT (I8,11I6)
c         i6 -> i8 for number of knots
6686  FORMAT (4I6,I8,I6)
c end Yannic
