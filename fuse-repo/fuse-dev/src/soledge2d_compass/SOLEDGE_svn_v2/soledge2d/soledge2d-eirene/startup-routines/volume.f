...
!      use soledge2eirene, only: vol_tri_eirene, voltot_eirene, Lz, 
!     . 				NTRI_soledge
...
c Yannick: check on areas

c        vol_diff(1:NTRII)=AREA(1:NTRII)*Lz-vol_tri(1:NTRII)

c        do IR=1,NTRII
c      		if (vol_diff(IR)>1.d-4) then
c      			write(iunout,*) 'inconsistency in triangle volume, ITRI = ',IR
c      			write(iunout,*) 'vol soledge = ',vol_tri(IR)
c        		write(iunout,*) 'vol_eirene  = ',AREA(IR)*Lz
c  		endif
c        enddo
...
c Yannick: get the cell volumes and total volume out in soledge2eirene module
!
!      voltot_eirene=0._dp
!
!      do IR=1,NTRII
