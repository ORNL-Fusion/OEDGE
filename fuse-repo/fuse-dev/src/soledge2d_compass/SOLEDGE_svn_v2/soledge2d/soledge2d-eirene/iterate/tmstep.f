...
c output to census.out removed, error in fzrtra (learca). Because no z mesh ? 

c        OPEN (UNIT=2,FILE='census.out')
c        WRITE (2,'(I9)') IPRNL
c        DO I=1,IPRNL
c          ITYP=ISPEZI(IPART(9,I),-1)
c          IION=ISPEZI(IPART(9,I),3)
c          XW=RPART(1,I)
c          YW=RPART(2,I)
c          ZW=RPART(3,I)
c          PH=RPART(11,I)
c          CALL EIRENE_FZRTRA(XW,ZW,PH,NT)
c          XW=XW+RMTOR
c          RW=SQRT(XW**2+ZW**2)
c          XW=RW*COS(PH)
c          ZW=RW*SIN(PH)
...
c          WRITE (2,'(4ES16.7,2X,2I6)') 
c     .           XW,YW,ZW,RPART(9,I),NMASSI(IION),NCHRGI(IION)
c        END DO
c        CLOSE (UNIT=2)
      ENDIF
C
