...
c yannick, crash at line 989, message not received because process are noy synchro ...
c this part is a mess because we rely on NMODE>0 to get infcop called ...
c arrays pertaining to B2 are allocated l964/965 only for my_pe>0
c temporary solution: suppress this, not used anyway
 
c      IF ((NMODE .NE. 0) .AND. (MY_PE .NE. 0)) THEN
c         CALL EIRENE_ALLOC_CCOUPL(1)
c         CALL EIRENE_ALLOC_CCOUPL(2)
c      END IF

c      IF (ALLOCATED(RCCPL)) THEN
c        CALL MPI_BCAST (RCCPL,NCOUPL,MPI_REAL8,0,MPI_COMM_WORLD,ier)
c        CALL MPI_BCAST (ICCPL1,MCOUPL1,MPI_INTEGER,0,MPI_COMM_WORLD,ier)
c        CALL MPI_BCAST (ICCPL2,MCOUPL2,MPI_INTEGER,0,MPI_COMM_WORLD,ier)
c        CALL MPI_BCAST (LCCPL,LCOUPL,MPI_LOGICAL,0,MPI_COMM_WORLD,ier)
c      END IF
c      CALL MPI_BCAST (B2BREM,1,MPI_REAL8,0,MPI_COMM_WORLD,ier)
c      CALL MPI_BCAST (B2RAD,1,MPI_REAL8,0,MPI_COMM_WORLD,ier)
c      CALL MPI_BCAST (B2QIE,1,MPI_REAL8,0,MPI_COMM_WORLD,ier)
c      CALL MPI_BCAST (B2VDP,1,MPI_REAL8,0,MPI_COMM_WORLD,ier)
c      IF (ALLOCATED(NAINS)) THEN
c        CALL MPI_BCAST (NAINS,NAIN,MPI_INTEGER,0,MPI_COMM_WORLD,ier)
c        CALL MPI_BCAST (NAINT,NAIN,MPI_INTEGER,0,MPI_COMM_WORLD,ier)
c      END IF
