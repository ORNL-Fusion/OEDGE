...
*******************************************************************************
      ...
      INTEGER FUNCTION EIRENE_P01ABE(IFAIL,IERROR,SRNAME,NREC,REC)
...
C     .. External Subroutines ..
      EXTERNAL                EIRENE_ABZP01, EIRENE_X04AAE, 
     .                        EIRENE_X04BAE
...
C           Noisy exit
            CALL EIRENE_X04AAE(0,NERR)
            DO 20 I = 1, NREC
               CALL EIRENE_X04BAE(NERR,REC(I))
   20       CONTINUE
            IF (IFAIL.NE.-13) THEN
               WRITE (MESS,FMT=99999) SRNAME, IERROR
               CALL EIRENE_X04BAE(NERR,MESS)
               IF (ABS(MOD(IFAIL,10)).NE.1) THEN
C                 Hard failure
                  CALL EIRENE_X04BAE(NERR,
