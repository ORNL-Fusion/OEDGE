...
C         pro(j)=p0+(ip-1)/float(np2nd-2)*(p1-p0)
          pro(j)=p0*exp(-(psurf(np2nd)-psurf(ip+1)+
     .          (psurf(ip+1)-psurf(ip))/2)/p1)
        enddo
        enddo
        enddo
...
!pb      call fzrtra (xx0,zz0,pphi,nnt)
      ...
      irusr = EIRENE_LEARC1(XX0,YY0,ZZ0,IPOLG,1,
     .       NR1STM,.FALSE.,.FALSE.,NPANU,
     .               'SM1USR      ')
...
C
      ...
      integer :: itc, irc, il, im, iu, icell
      real(dp) :: rad
...
C
      ...
      entry EIRENE_PNTUSR(T,X,Y,Z,IND,P,Q)
        X=5.+T
        Y=20.
        Z=2.5
      RETURN
      entry EIRENE_DPTUSR(T,DX,DY,DZ,IND,P,Q)
        DX=1.
        DY=0.
        DZ=0.
      RETURN
      entry EIRENE_DDPUSR(T,DDX,DDY,DDZ,IND,P,Q)
        DDX=0.
        DDY=0.
        DDZ=0.
      RETURN
