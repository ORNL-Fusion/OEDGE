...
C     IF (NLTRC) THEN
C       CALL LEER(1)
C       WRITE (iunout,*) 'TIMEA ,X,Y,Z,T ',X,Y,Z,T
C       WRITE (iunout,*) '      VX,VY,VZ,V ',VX,VY,VZ,V
...
C  NO
     ...
     .            +(A3LM(J)+A6LM(J)*Z)*Z
        ...
        TMA=F+G
        TMI=F-G
C       IF (NLTRC) WRITE (iunout,*) 'TIMEA, J,TMI,TMA ',J,TMI,TMA
...
40      TMA=-A2/A1
C       IF (NLTRC)
C     .   WRITE (iunout,*) 'TIMEA AT 40, J,TMA,A1,A2 ',J,TMA,A1,A2
        IF (TMA.LE.EPS12) GOTO 100
        IF (TMA.GT.TMIN) THEN
          IF (.NOT.NLPRCS(J)) THEN
            GOTO 100
...
50      IF (A2.EQ.0.) GOTO 100
        TMA=-A3/A2
C       IF (NLTRC) WRITE (iunout,*) 'TIMEA AT 50, J,TMA,A2,A3 ',
C     .                                           J,TMA,A2,A3
        IF (TMA.LE.EPS12) GOTO 100
        IF (TMA.GT.TMIN) THEN
          IF (.NOT.NLPRCS(J)) THEN
...
70      TL=TMX+TADD
        ...
        SG=SIGN(1._DP,WR)
C       IF (NLTRC) THEN
C         WRITE (iunout,*) 'TIMEA, TL,XR,YR,ZR,NNR,MASURF,SG '
C         WRITE (iunout,*)         TL,XR,YR,ZR,NNR,MASURF,SG
C       ENDIF
C
...
C
C     IF (NLTRC) WRITE (iunout,*) 'NOT RETURNED FROM TIMEA, OTHER LOOP '
