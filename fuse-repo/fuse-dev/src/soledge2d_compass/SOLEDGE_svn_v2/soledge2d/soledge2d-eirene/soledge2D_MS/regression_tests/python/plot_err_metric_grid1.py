...
# cpp
    ...
    n2=dataset[...]
    error=np.sum(abs(n2[0:Nx-1,0:Nx/2-1]-n[1:Nx,1:Nx/2]))/((Nx/2-1)*Nx)
    error_rel=np.sum(abs(n2[0:Nx-1,0:Nx/2-1]-n[1:Nx,1:Nx/2])/abs(n[1:Nx,1:Nx/2]))/((Nx/2)*Nx)
    err_cpp[k-2]=error
...
# ctt
    ...
    n2 = np.zeros((Nx/2,Nx,), dtype=float)
    n2=dataset[...]
    error=np.sum(abs(n2[0:Nx-1,0:Nx/2-1]-n[1:Nx,1:Nx/2]))/((Nx/2)*Nx)
    error_rel=np.sum(abs(n2[0:Nx-1,0:Nx/2-1]-n[1:Nx,1:Nx/2])/abs(n[1:Nx,1:Nx/2]))/((Nx/2)*Nx)
    err_ctt[k-2]=error
...
plt.legend([line_n, line_n3, line_n4, line_n5],['cpp','ctt','Jac','G'])
plt.title('Metric - Absolute error / Grid1')
plt.xlabel('Grid resolution')
plt.ylabel('Truncation error')
plt.savefig('errors/err_abs_metric_grid1.png')
...
plt.grid(True,which="both",ls="-")
plt.legend([line_n, line_n3, line_n4, line_n5],['cpp','ctt','Jac','G'])
plt.title('Metric - Relative error / Grid1')
plt.xlabel('Grid resolution')
plt.ylabel('Truncation error')
plt.savefig('errors/err_rel_metric_grid1.png')
