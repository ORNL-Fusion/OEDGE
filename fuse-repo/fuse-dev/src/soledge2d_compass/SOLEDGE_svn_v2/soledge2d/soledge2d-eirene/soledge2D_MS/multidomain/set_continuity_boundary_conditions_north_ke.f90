subroutine set_continuity_boundary_conditions_north_ke(zone,STEP)
#include "compile_opt.inc"
  ...
  zone%kepsilon(STEP)%k(Nx+1,1:Nz) = zone%kepsilon(STEP)%k(Nx,1:Nz)
  zone%kepsilon(STEP)%epsilon(Nx+1,1:Nz) = zone%kepsilon(STEP)%epsilon(Nx,1:Nz)
end subroutine set_continuity_boundary_conditions_north_ke
