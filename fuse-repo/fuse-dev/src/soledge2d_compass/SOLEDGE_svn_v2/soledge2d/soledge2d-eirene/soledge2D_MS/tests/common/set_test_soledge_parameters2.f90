subroutine set_test_soledge_parameters2()
  use all_variables
  implicit none
  !####################################################
  !soledge parameters
  ...
  drift_flags%solve_phi=.false.
  drift_flags%solve_drift=.false.
  flags%neutral_model=0
  flags%restart=.false.
  flags%use_WENO=.true.
  global_variables%epsilon_weno=1.d-6
  ...
  global_parameters%element_list(1)%symbol='He'
  ...
  boundary_conditions%BCTe=100.D0
  boundary_conditions%BCTi(1)=100.D0
  !###################################################
...
end subroutine set_test_soledge_parameters2
