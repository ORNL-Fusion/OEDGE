program test_drift_term
#include "compile_opt.inc"
  use test_var
  use all_variables
  use init_test_mod_drift
#if GFORTRAN == 0
  use ifport
#endif
  implicit none
  ...
  !#############################################################
  call init_test_drift(level,'D ',100.d0,1)
  ...
  call compute_test_drift_sources()

  call step_in_soledge_drift()

  call save_plasma_test(level)
  call save_drift_test(level)  

end program test_drift_term
