...
errn=np.zeros(npt, dtype=float)
errnr=np.zeros(npt, dtype=float)
errG=np.zeros(npt, dtype=float)
errGr=np.zeros(npt, dtype=float)
errT=np.zeros(npt, dtype=float)
errTe=np.zeros(npt, dtype=float)
errTr=np.zeros(npt, dtype=float)
errTer=np.zeros(npt, dtype=float)
res=np.zeros(npt, dtype=float)

dt=1E-5

for k in range(2,npt+2):
    res[k-2]=2**(k+1)
    Nx=2**(k+1)
...
plt.title('Perpendicular diffusion - Absolute error / Grid1')
plt.xlabel('Grid resolution')
plt.ylabel('Truncation error')
plt.savefig('errors/err_abs_perp_grid1.png')
...
plt.grid(True,which="both",ls="-")
plt.legend([line_n, line_G, line_T, line_Te],['Density','Velocity','Ti','Te'])
plt.title('Perpendicular diffusion - Relative error / Grid1')
plt.xlabel('Grid resolution')
plt.ylabel('Truncation error')
plt.savefig('errors/err_rel_perp_grid1.png')
