subroutine apply_velocity_boundary_conditions(flux_surface,n_ion)
  ...
  if(.not.flux_surface%properties%is_periodic) then
     ...
     elseif (zones(izone)%Neighbors(4) .eq. -6) then
        flux_surface%tridiag%s(1)=flux_surface%tridiag%s(1)-&
             flux_surface%tridiag%a(1)*zones(izone)%species(n_ion)%var(1)%Gamma(i_psi,0)
     else
        Mach=zones(izone)%species(n_ion)%var(1)%Mach(i_psi,1)
        if(global_variables%sign_metric.eq.1.d0) then
           if(Mach.ge.-1.d0) then !if M<-1 then M=-1
           ...
           if(Mach.le.1.d0) then !if M<-1 then M=-1
              flux_surface%tridiag%S(1)=flux_surface%tridiag%S(1)-flux_surface%tridiag%a(1)*&
                   zones(izone)%species(n_ion)%var(1)%density(i_psi,0)&
                   *sqrt((zones(izone)%species(n_ion)%var(1)%temperature(i_psi,0)+&
                   zones(izone)%species(0)%var(1)%temperature(i_psi,0))/zones(izone)%species(n_ion)%element%mass)
     ...
     elseif (zones(izone)%Neighbors(3).eq.-6) then
        flux_surface%tridiag%s(Nz)=flux_surface%tridiag%s(Nz)-&
             flux_surface%tridiag%c(Nz)*zones(izone)%species(n_ion)%var(1)%Gamma(i_psi,Nz_N+1)
     else
        Mach=zones(izone)%species(n_ion)%var(1)%Mach(i_psi,Nz_N)
        if(global_variables%sign_metric.eq.1.d0) then
           if(Mach.le.1.d0) then
