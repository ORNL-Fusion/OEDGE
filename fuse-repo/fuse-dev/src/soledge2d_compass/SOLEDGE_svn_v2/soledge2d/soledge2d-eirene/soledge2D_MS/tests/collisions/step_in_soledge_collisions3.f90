subroutine step_in_soledge_collisions3()
  use all_variables
  use test_var
  use Mdefinitions
  implicit none
...
!     call add_velocity_penalisation_terms(zones(k))
     call add_velocity_coupling_terms(zones(k))     
...
!     call add_temperature_Eparallel_term(zones(k))
     call add_temperature_coupling_terms(zones(k)) 
...
end subroutine step_in_soledge_collisions3
