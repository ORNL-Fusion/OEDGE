subroutine shape1chi(bchis,bNx,bNz,begins,ends,nbeg,ind) ! XX\__/XX\__
  implicit none
  integer*4 bNx,bNz
  integer*4,dimension(1:4,1:bNx,1:bNz):: bchis
  integer*4,dimension(1:bNx)::nbeg
  ...
  !mask at zone beginning
  length=ends(ind,1)
  if(length.ge.3) then
     l1=min(1,length/3)
     l2=length-l1
     do j=ends(ind,1)-l1+1,ends(ind,1)
        bchis(3,ind,j)=1.D0
     end do
     do j=l1,l2
        bchis(4,ind,j)=1.D0
