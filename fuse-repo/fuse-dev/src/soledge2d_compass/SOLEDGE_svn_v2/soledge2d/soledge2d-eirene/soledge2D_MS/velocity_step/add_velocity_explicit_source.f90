subroutine add_velocity_explicit_source(zone)
...
end subroutine add_velocity_explicit_source


subroutine add_velocity_explicit_source_point(zone,i,j,n)
  use all_variables, only : global_parameters, reference_parameters,&
       global_variables, penalisation_parameters
  use Mzone
  use Mphysics
  ...
  zone%species(n)%tridiag%S(i,j)=zone%species(n)%tridiag%S(i,j)&
       +zone%species(n)%sources%SG(i,j)
end subroutine add_velocity_explicit_source_point
