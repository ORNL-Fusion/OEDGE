subroutine compute_explicit_gradB_drift_perp_source_terms(zone)
  ...
  do n=0,global_parameters%N_ions
     !! Curvature drift
     v_psi=zone%species(n)%drifts%uBp*(2.d0*pi*R0/rs0)
     v_theta=0.d0!2.d0*pi*R0/c0*(zone%species(n)%drifts%uBt)/(2.d0*pi)
...
!!$     if(n.eq.1) then
!!$        write(filename,"(A4,I0)") "SBr_",zone%number
...
end subroutine compute_explicit_gradB_drift_perp_source_terms
