subroutine load_kepsilon_fields()
  ...
  do k=1,global_parameters%N_zones
     ...
     dim2d(1)=Nx+2
     dim2d(2)=Nz+2
     call h5dopen_f(group_id,"k",dataset_id,error)
     call h5dread_f(dataset_id,H5T_NATIVE_DOUBLE,buffer,dim2d,error)
     zones(k)%kepsilon(1)%k=buffer
     call h5dclose_f(dataset_id,error)
     call h5dopen_f(group_id,"epsilon",dataset_id,error)
     call h5dread_f(dataset_id,H5T_NATIVE_DOUBLE,buffer,dim2d,error)
     zones(k)%kepsilon(1)%epsilon=buffer
     call h5dclose_f(dataset_id,error)
     deallocate(buffer)
...
end subroutine load_kepsilon_fields
