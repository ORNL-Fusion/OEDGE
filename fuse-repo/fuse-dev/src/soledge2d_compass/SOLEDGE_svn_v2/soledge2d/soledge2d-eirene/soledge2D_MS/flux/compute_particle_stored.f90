subroutine compute_particle_stored(zone,nion,Source)
  use MZone
  use all_variables, only : reference_parameters, flags, global_parameters
  implicit none
  ...
  do i=1,Nx
     do j=1,Nz
        Source=Source+(zone%species(nion)%var(2)%density(i,j)&
              )*zone%metric_coefficients%dvol_pu(i,j)&
             *n0&
             *(1.d0-zone%masks%chi2(i,j))
     end do
  end do
end subroutine compute_particle_stored
