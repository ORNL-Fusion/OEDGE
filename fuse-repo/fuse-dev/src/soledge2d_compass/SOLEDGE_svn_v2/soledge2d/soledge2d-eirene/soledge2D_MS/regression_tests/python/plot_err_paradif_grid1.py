import h5py
import numpy as np

npt=6

errn=np.zeros(npt, dtype=float)
errG=np.zeros(npt, dtype=float)
errT=np.zeros(npt, dtype=float)
errTe=np.zeros(npt, dtype=float)
errTr=np.zeros(npt, dtype=float)
errTer=np.zeros(npt, dtype=float)
res=np.zeros(npt, dtype=float)

dt=1E-5

for k in range(2,npt+2):
    res[k-2]=2**(k+1)
    Nx=2**(k+1)
    f=h5py.File('Results/'+str(k)+'_plasma_1','r')
    dataset = f['/zone1/temperature']
    n = np.zeros((Nx/2+2,Nx+2,), dtype=float)
    n=dataset[...]
    dataset = f['/zone1/temperature2']
    n2 = np.zeros((Nx/2+2,Nx+2,), dtype=float)
    n2=dataset[...]
    error=np.sum(abs(n2[1:Nx,1:Nx/2]-n[1:Nx,1:Nx/2]))/(Nx**2/2)
    error_rel=np.sum(abs(n2[1:Nx,1:Nx/2]-n[1:Nx,1:Nx/2])/abs(n[1:Nx,1:Nx/2]))/(Nx**2/2)
    errT[k-2]=error/dt
    errTr[k-2]=error_rel/dt
    f.close()

    f=h5py.File('Results/'+str(k)+'_plasma_0','r')
    dataset = f['/zone1/temperature']
    n = np.zeros((Nx/2+2,Nx+2,), dtype=float)
    n=dataset[...]
    dataset = f['/zone1/temperature2']
    n2 = np.zeros((Nx/2+2,Nx+2,), dtype=float)
    n2=dataset[...]
    error=np.sum(abs(n2[1:Nx,1:Nx/2]-n[1:Nx,1:Nx/2]))/(Nx**2/2)
    error_rel=np.sum(abs(n2[1:Nx,1:Nx/2]-n[1:Nx,1:Nx/2])/abs(n[1:Nx,1:Nx/2]))/(Nx**2/2)
    errTe[k-2]=error/dt
    errTer[k-2]=error_rel/dt
    f.close()


import matplotlib as mpl
mpl.use('Agg')
from pylab import *
import matplotlib.pyplot  as plt
figure(0)
line_T,= plt.loglog(res,errT, 'b^-', lw=2)
line_Te,= plt.loglog(res,errTe, 'ro-', lw=2)
plt.grid(True,which="both",ls="-")
plt.legend([line_T, line_Te],['Ti','Te'])
plt.title('Parallel diffusion - Absolute error / Grid1')
plt.xlabel('Grid resolution')
plt.ylabel('Truncation error')
plt.savefig('errors/err_abs_para_grid1.png')

figure(1)
line_T,= plt.loglog(res,errTr, 'b^-', lw=2)
line_Te,= plt.loglog(res,errTer, 'ro-', lw=2)
plt.grid(True,which="both",ls="-")
plt.legend([line_T, line_Te],['Ti','Te'])
plt.title('Parallel diffusion - Relative error / Grid1')
plt.xlabel('Grid resolution')
plt.ylabel('Truncation error')
plt.savefig('errors/err_rel_para_grid1.png')
