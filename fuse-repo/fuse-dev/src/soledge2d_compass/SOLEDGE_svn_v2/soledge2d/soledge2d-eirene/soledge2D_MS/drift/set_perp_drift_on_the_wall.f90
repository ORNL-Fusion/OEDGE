subroutine set_perp_drift_on_the_wall(zone,Fluxes,Nx,Nz)
  use MZone
  implicit none
  ...
  if(zone%Neighbors(3).eq.-3) then
     do i=1,Nx
        Fluxes(i,Nz,3)=Fluxes(i,Nz,4)
     end do
  end if
  !west
  if(zone%Neighbors(4).eq.-3) then
     do i=1,Nx
        Fluxes(i,1,4)=Fluxes(i,1,3)
     end do
  end if
end subroutine set_perp_drift_on_the_wall
