subroutine compute_friction_force(zone)
  ...
  real*8 :: density_mass,mass_ratio
  real*8,parameter :: coef=-0.51D0
  Nx=zone%mesh%Nx
  Nz=zone%mesh%Nz
  do i=1,Nx
     do j=1,Nz
        do n=0,global_parameters%N_ions
           do m=0,global_parameters%N_ions
              ...
              mass_ratio=(zone%species(n)%element%mass2/zone%species(m)%element%mass2&
                   +zone%species(m)%element%mass2/zone%species(n)%element%mass2)*0.5D0
              zone%species(n)%coupling_terms%R(i,j,m)=coef*density_mass*mass_ratio/&
...
end subroutine compute_friction_force
