...
C
      ...
      INTEGER :: NTR, I, J, LL, nix1, nix2
      REAL(DP), ALLOCATABLE, SAVE :: PLAS(:,:),EF(:,:)
      REAL(DP) :: bnorm
      INTEGER, SAVE :: INDAR(10)=(/ (0, i=1,10) /),ic 
...
!pb	close(31)
      ...
      ELSEIF (INDX == 4+5*NPLS) THEN
        ...
        END IF
        INDAR(10) = INDAR(10) + 1
      ELSEIF (INDX == 6+1*NPLS+NPLSTI+3*NPLSV) THEN
        ! lese elektrisches potential  
        
	if ( .not.allocated(EF) ) then
	
	  allocate(EF(4,n1st))
	
	  LL=LEN_TRIM(CASENAME)

          FILENAME=CASENAME(1:LL) // '.efield'
	...
	if (ic>4) then
	  write(*,*) 'PROUSR: NO DATA PROVIDED FOR ADIN(',ic,',:)'
	  PRO(1:N) = 0._DP
	else	
          PRO(1:NTR) = EF(ic,1:NTR)
	  ic = ic + 1
        endif
