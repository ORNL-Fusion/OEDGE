...
for k=1:nzones
   ...
   for i=1:zone(k).Nx
       for j=1:zone(k).Nz
            %segment1
            P1_R=zone(k).Rtri(i,j);
            P1_Z=zone(k).Ztri(i,j);
            P2_R=zone(k).Rtri(i+1,j);
            P2_Z=zone(k).Ztri(i+1,j);
            if(inpolygon(P1_R,P1_Z,WEST_paroi(:,1),WEST_paroi(:,2))-inpolygon(P2_R,P2_Z,WEST_paroi(:,1),WEST_paroi(:,2))~=0)
                zone(k).iscrossed(i,j)=1;
            end
            %segment2
            P1_R=zone(k).Rtri(i+1,j);
            P1_Z=zone(k).Ztri(i+1,j);
            P2_R=zone(k).Rtri(i+1,j+1);
            P2_Z=zone(k).Ztri(i+1,j+1);
            if(inpolygon(P1_R,P1_Z,WEST_paroi(:,1),WEST_paroi(:,2))-inpolygon(P2_R,P2_Z,WEST_paroi(:,1),WEST_paroi(:,2))~=0)
                zone(k).iscrossed(i,j)=1;
            end
            %segment3
            P1_R=zone(k).Rtri(i+1,j+1);
            P1_Z=zone(k).Ztri(i+1,j+1);
            P2_R=zone(k).Rtri(i,j+1);
            P2_Z=zone(k).Ztri(i,j+1);
            if(inpolygon(P1_R,P1_Z,WEST_paroi(:,1),WEST_paroi(:,2))-inpolygon(P2_R,P2_Z,WEST_paroi(:,1),WEST_paroi(:,2))~=0)
                zone(k).iscrossed(i,j)=1;
            end
            %segment4
            P1_R=zone(k).Rtri(i,j+1);
            P1_Z=zone(k).Ztri(i,j+1);
            P2_R=zone(k).Rtri(i,j);
            P2_Z=zone(k).Ztri(i,j);
            if(inpolygon(P1_R,P1_Z,WEST_paroi(:,1),WEST_paroi(:,2))-inpolygon(P2_R,P2_Z,WEST_paroi(:,1),WEST_paroi(:,2))~=0)
                zone(k).iscrossed(i,j)=1;
            end
       end
