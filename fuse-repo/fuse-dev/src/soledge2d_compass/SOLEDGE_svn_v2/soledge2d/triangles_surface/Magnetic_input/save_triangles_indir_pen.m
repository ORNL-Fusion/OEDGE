...
for i=1:nzones
    ...
    for k=1:Nx
        for j=1:Nz
            ...
            tri_knots=[tri_knots;nt1,nt3,nt2];
            
            tri_info(ntriangle).k=i;
            tri_info(ntriangle).i=k;
            tri_info(ntriangle).j=j;            
            tri_info(ntriangle).p1=nt1; 
            tri_info(ntriangle).p2=nt3; 
            tri_info(ntriangle).p3=nt2; 
            
            
            zone(i).triangles(k,j,1)=ntriangle;
            zone(i).trivert(k,j,1,1)=nt1;
            zone(i).trivert(k,j,1,2)=nt3;
            zone(i).trivert(k,j,1,3)=nt2;
            ...
            zone(i).trivert(k,j,2,2)=nt3;
            zone(i).trivert(k,j,2,3)=nt2;
...
for i=1:nzones
   ...
   for k=1:Nx
       for j=1:Nz
           ...
           if(k==1)
                if(B(2)==1) %BC au sud 
                   zone(i).neightri(k,j,2,1)=0;
                   zone(i).neighsid(k,j,2,1)=0;
                   zone(i).neightyp(k,j,2,1)=2;
                else
                   zone(i).neightri(k,j,2,1)=zone(A(2)).triangles(zone(A(2)).Nx,j,1);
                   zone(i).neighsid(k,j,2,1)=2;
                   zone(i).neightyp(k,j,2,1)=0;
                end
            else
                zone(i).neightri(k,j,2,1)=zone(i).triangles(k-1,j,1);
                zone(i).neighsid(k,j,2,1)=2;
                zone(i).neightyp(k,j,2,1)=0;
           end
            ...
            if(j==1) 
               if(B(4)==1) %BC à l'ouest
                   zone(i).neightri(k,j,1,3)=0;
                   zone(i).neighsid(k,j,1,3)=0;
                   zone(i).neightyp(k,j,1,3)=2;
               else
                   zone(i).neightri(k,j,1,3)=zone(A(4)).triangles(k,zone(A(4)).Nz,2);
                   zone(i).neighsid(k,j,1,3)=2;
                   zone(i).neightyp(k,j,1,3)=0;
               end
            else
                zone(i).neightri(k,j,1,3)=zone(i).triangles(k,j-1,2);
                zone(i).neighsid(k,j,1,3)=2;
                zone(i).neightyp(k,j,1,3)=0;
            end
           ...
           % k+1,j neighbor
            if(k==Nx)
               if(B(1)==1) %BC au nord
                  ...
                  zone(i).neightyp(k,j,1,2)=2;
               else
                  zone(i).neightri(k,j,1,2)=zone(A(1)).triangles(1,j,2);
                  zone(i).neighsid(k,j,1,2)=1;
            ...
            if(j==Nz)
                if(B(3)==1) %BC a l'est
                    ...
                    zone(i).neightyp(k,j,2,2)=2;
                else
                    zone(i).neightri(k,j,2,2)=zone(A(3)).triangles(k,1,1);
                    zone(i).neighsid(k,j,2,2)=3;
            ...
            zone(i).neightri(k,j,1,1)=zone(i).triangles(k,j,2);
            zone(i).neighsid(k,j,1,1)=3;
            zone(i).neightri(k,j,2,3)=zone(i).triangles(k,j,1);
            zone(i).neighsid(k,j,2,3)=1;
       end
   end
end
   fclose(fid);
