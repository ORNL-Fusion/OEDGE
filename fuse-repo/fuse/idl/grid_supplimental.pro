...
; getb,'jet-psin_test.cell',machine='EQU',equ='/home/ITER/lisgos/divimp/shots/jet/80295/jet_80295_18600.16x.equ',/CHANGE_SIGN
;
; ======================================================================
;
...
;  print,min(psi+psib-psi_min),max(psi+psib-psi_min)
  ...
  min_psi = MIN(-b.psi)             
  psin = 1.0D + psi / min_psi   ; changed 05/03/2012 to match what's in grid_write.pro, in GRID
...
; ----------------------------------------------------------------------
  'EQU': BEGIN
      b = grid_ReadEquFile(equ,CHANGE_SIGN=change_sign)
    END
; ----------------------------------------------------------------------
  'MAST': BEGIN
    print,shot
...
;
  ...
  ; jrh mod begin
  b_tot = MAKE_ARRAY(grid.n,/DOUBLE,VALUE=0.0)
  FOR i = 0, grid.n-1 DO BEGIN
    b_tot[i] = Map2D(b.b_total,b.x,b.y,grid.x_mid[i],grid.y_mid[i],cubic=-0.5)
  ENDFOR
  grid = CREATE_STRUCT(grid,'b_total',b_tot)
  ; jrh mod end
...
;                   format='(/4A10,2A6,2X,3A12,2X,3A12)'
    ; jrh mod begin - add b_tot information in output
    ; old
;    PRINTF,fpout,'* ','IK','IR','PSIn','B_ratio',  $
;                   format='(A2,2A6,2A14)'
    ; new
     PRINTF,fpout,'* ','IK','IR','PSIn','B_ratio','PSI','B_tot',  $
                    format='(A2,2A6,2A14,2A14)'
...
;                   FORMAT='(2X,2I6,3F14.7)'
       ; jrh mod begin
      PRINTF,fpout,grid.ik[i],grid.ir[i],grid.psin[i], grid.b_ratio[i], $
                   grid.psi[i],  grid.b_total[i], $  ; *** TEMP ***
                   FORMAT='(2X,2I6,3F14.7,3F14.7)'
       ; jrh mod end
    ENDFOR
