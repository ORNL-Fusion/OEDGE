...
;    ypos  = [ycen - 0.45 * ysize, ycen + 0.5  * ysize]
    ...
    FOR idata = 1, ndata DO BEGIN
      ...
      IF (N_ELEMENTS(WHERE(plot.xrange EQ 0.0)) NE 2) THEN BEGIN
        ...
        IF (iplot EQ 6) THEN ymax = MAX([ymax,val.ti[i]])
      ENDIF ELSE BEGIN
        xmin = MIN([xmin,xdata])
        xmax = MAX([xmax,xdata])
        ymin = MIN([ymin,ydata])
        ymax = MAX([ymax,ydata])
        IF (iplot EQ 6) THEN ymax = MAX([ymax,val.ti])
      ENDELSE
    ENDFOR
...
print,val.dens
    ...
    CASE (iplot) OF
      ...
      2    : PLOT, xrange, yrange, /NODATA, XSTYLE=1, YSTYLE=1,  $ YLOG=1,  $
      ...
      5    : PLOT, xrange, yrange, /NODATA, XSTYLE=1, YSTYLE=1,  $
                   POSITION=position,YTITLE=plot_ytitle[iplot-1],XTICKFORMAT='(A1)',/NOERASE
      6    : PLOT, xrange, yrange, /NODATA, XSTYLE=1, YSTYLE=1,  $ YLOG=1,  $
;      6    : PLOT, xrange, yrange, /NODATA, XSTYLE=1, YSTYLE=1, YLOG=1,  $
                   POSITION=position,YTITLE=plot_ytitle[iplot-1],XTITLE=plot_xtitle,/NOERASE                                 
    ENDCASE
...
; list hack
;      IF (idata EQ ndata) THEN BEGIN
;        OPLOT, xdata, ydata, COLOR=TrueColor(colors[1])
;      ENDIF ELSE BEGIN
;        OPLOT, xdata, ydata, COLOR=TrueColor(colors[0])
;      ENDELSE
