...
; DEVICE, SET_FONT='Courier', /TT_FONT
  ...
  xtitle = 'bin'
  ytitle = [' ','edge impurity concentration (%)','3','4','5','6']
...
;...
    ...
    ENDIF ELSE BEGIN
      ...
      FOR idata = 1, ndata DO BEGIN
        ...
        xmin = MIN([xmin,FLOAT(val.time_slice*10)])
        xmax = MAX([xmax,FLOAT(val.time_slice*10)])
      ENDFOR
      deltax = xmax - xmin
...
;   Axes:
    ...
    CASE (iplot) OF
      focus: PLOT, xrange, yrange, /NODATA, XSTYLE=1, YSTYLE=1, YLOG=ylog, XMINOR=1,  $
                   POSITION=position,YTITLE=ytitle[iplot-1],CHARSIZE=charsize,XTITLE='time (us)'
...
; ======================================================================
;
FUNCTION cortex_PlotSummary_Original, plot, plot_array, ps=ps
