      ...
      DO IX=0,NDXP
        ...
        DO IPRT=1,NPLP-1
          ...
          IF (IX.GE.IXI.AND.IX.LT.IXE) THEN
            LCUT(IX)=.TRUE.
            WRITE (6,*) 'POLOIDAL CUT CELL INTRODUCED AT IP= ',IX
          ENDIF
        ENDDO
      ENDDO
      CALL LEER(1)
...
C
      IF (NTRII.GT.NRAD.OR.NTRII.GT.NTRI) THEN
        WRITE (6,*) ' PARAMETER ERROR DETECTED IN INFUSR '
        WRITE (6,*) ' NTRII MUST BE < NRAD AND <= NTRI'
        WRITE (6,*) ' NTRII,NRAD,NTRI = ',NTRII,NRAD,NTRI
        CALL EXIT
      ENDIF
      DO I=1,NTRII
...
C
     >               IXTRI(J),IYTRI(J)
C       WRITE(6,*) J,NECKE(1,J),NECKE(2,J),NECKE(3,J),
...
C
      ...
      DO I=1,NADMOD
        ...
        WRITE (6,*) 'WRONG POINTNUMBER IN INFCOP '
...
C
      ...
      WRITE (6,*) 'NR1TAL,NP2TAL,NSBOX_TAL ',NR1TAL,NP2TAL,NSBOX_TAL
      CALL LEER(2)
CTRIG E
      RETURN
C
...
C  THIS MEANS ONE SIDE OF TRIANGLE BELONGS TO TARGET
                ...
                IF (ITRI.GT.NGITT) THEN
                  WRITE (6,*) ' NOT ENOUGH GRIDPOINTS FOR DEFINING',
     .                        ' STEP-FUNCTION '
                  WRITE (6,*) ' INCREASE PARAMETER NGITT '
                  WRITE (6,*) ' NGITT = ',NGITT
                  CALL EXIT
                ENDIF
                NUMTRI(ITRI)=IT
...
C  EIRENE CELL NUMBER INFORMATION ALONG TARGET
          ...
          IF (INMTI(IS,ITRI).EQ.0) THEN
            WRITE (6,*) 'ERROR IN INFCOP '
            WRITE (6,*) 'SOURCE NOT ON A KNOWN SURFACE'
            WRITE (6,*) 'ITARG,IG,IPRT ',ITARG,IG,IPRT
          ENDIF
          ...
          DO 7473 IPLS=1,NPLSI
            ...
            DO 7472 IIRC=1,NPRCI(IPLS)
              ...
              DO 7471 IT=1,NTRII
                ...
                COPV(ICPV,IT)=COPV(ICPV,IT)+PIADD
            ...
            DO 7536 IX=1,NDXA
              DO 7533 IY=1,NDYA
                ...
                DO WHILE (ASSOCIATED(CURPOI))
                  ...
                  SMOCL=COPV(IADD+IPLS,IT)*VOLTAL(IT)*1.D-5*SIGNUM
...
C  ITARG, IPRT KNOWN FROM ABOVE
          ...
          DO 10112 IF=NSPZI(ITARG,IPRT),NSPZE(ITARG,IPRT)
            ...
            IF (NINCT(ITARG,IPRT)*FNIYB(IX,0,IF).LT.0) THEN
              ...
              WRITE (6,*) 'SOUTH,IX,ITARG,IPRT,IF ',IX,ITARG,IPRT,IF
...
C  ITARG, IPRT KNOWN FROM ABOVE
          ...
          DO 10117 IF=NSPZI(ITARG,IPRT),NSPZE(ITARG,IPRT)
            ...
            IF (NINCT(ITARG,IPRT)*FNIYB(IX,NDYA,IF).LT.0) THEN
              WRITE (6,*) 'RECYCLING TARGET, BUT WRONG FLOW DIRECTION: '
              WRITE (6,*) 'NORTH,IX,ITARG,IPRT,IF ',IX,ITARG,IPRT,IF
...
C  ITARG, IPRT KNOWN FROM ABOVE
          ...
          DO 10122 IF=NSPZI(ITARG,IPRT),NSPZE(ITARG,IPRT)
            ...
            IF (NINCT(ITARG,IPRT)*FNIXB(0,IY,IF).LT.0) THEN
              WRITE (6,*) 'RECYCLING TARGET, BUT WRONG FLOW DIRECTION: '
              WRITE (6,*) 'WEST,IY,ITARG,IPRT,IF ',IY,ITARG,IPRT,IF
...
C  ITARG, IPRT KNOWN FROM ABOVE
          ...
          DO 10127 IF=NSPZI(ITARG,IPRT),NSPZE(ITARG,IPRT)
            ...
            IF (NINCT(ITARG,IPRT)*FNIXB(NDXA,IY,IF).LT.0) THEN
              WRITE (6,*) 'RECYCLING TARGET, BUT WRONG FLOW DIRECTION: '
              WRITE (6,*) 'EAST,IY,ITARG,IPRT,IF ',IY,ITARG,IPRT,IF
