...
C
     ...
     .             3,4,1/
      ...
      IF (NRK /= NCOORD) THEN
        WRITE (6,*) ' NCOORD IS WRONG IN EIRENE INPUT FILE'
      ...
      DO IT=1,NTET
        DO IS=1,4
          IF (NTBAR(IS,IT).EQ.0.AND.INMTIT(IS,IT).EQ.0) THEN
            WRITE (6,*) ' ERROR IN READ_TETRA '
            WRITE (6,*) ' OPEN SIDE OF TETRAHEDRON ',IT,' SIDE ',IS
            WRITE (6,*) ' NTECK ',(NTECK(ITSIDE(J,IS),IT),J=1,3)
            IER = 3
          ENDIF
        ENDDO
      ENDDO
