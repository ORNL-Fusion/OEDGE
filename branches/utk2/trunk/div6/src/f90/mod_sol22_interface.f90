...
contains
  ...
  subroutine calcsol_interface (irlim1,irlim2,ikopt)
    ...
    real te_smooth_fact1, te_smooth_fact2, ti_smooth_fact1, ti_smooth_fact2, n_smooth_fact1, n_smooth_fact2
...
500          CONTINUE
          ...
          if (switch(swsmooth).eq.1.0.or.switch(swsmooth).eq.2.0) then
			 ...
			 ! Variables used in both smoothing options.
	         ...
	         ! Smoothing option 1 variables. asmexp determines smoothing range.
             asmexp = 1.0
