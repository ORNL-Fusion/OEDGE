...
c
      ...
      INTEGER mode,index,i1,ik,ir,id,ike,ind1,in
      REAL*8  deltax,deltay,rv,zv,fx,fy,x1,x2,y1,y2,deltax1,deltax2

      IF (mode.EQ.-1) THEN
c...    ITER HFS wall morph for :
...
c...    Most of the wall:
        ...
        DO ir = 1, nrs
          ...
          DO ik = 1, ike
            id = korpg(ik,ir)
            DO i1 = 1, nvertp(id)
              rv = DBLE(rvertp(i1,id))
              zv = DBLE(zvertp(i1,id))
              IF (rv.GE.x1.AND.rv.LE.x2.AND.
     .            zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
                fx = 1.0 - (rv - x1) / (x2 - x1)
                fy = 1.0 
     ...
     .          zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
        ...
        DO id = 1, wallpts
          DO i1 = 1, 2
            rv = wallpt(id,20+2*(i1-1)  )
            zv = wallpt(id,20+2*(i1-1)+1)
            IF (rv.GE.x1.AND.rv.LE.x2.AND.
     .          zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
              fx = 1.0 - (rv - x1) / (x2 - x1)
              fy = 1.0 
              wallpt(id,20+2*(i1-1)  ) = rv + deltax * fx *fy
              wallpt(id,20+2*(i1-1)+1) = zv 
            ENDIF
          ENDDO
        ENDDO      
        DO in = 1, nds
...
c        x2 =  4.33
        ...
        deltax1 =  0.068  ! 0.06 > 4.098, 0.065 > 4.10283  (4.105)
        deltax2 =  0.099 ! 097 !   4.13183  0.085 >  4.12217   0.072 > 4.109                         (4.135) 
        deltay  =  0.0
     ...
     .            zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
                fx = 1.0 - (rv - x1) / (x2 - x1)
                fy =       (zv - y1) / (y2 - y1)
                deltax = (1.0 - fy) * deltax1 + fy * deltax2
     ...
     .          zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
              ...
              fy =       (zv - y1) / (y2 - y1)
              deltax = (1.0 - fy) * deltax1 + fy * deltax2
              wallpt(id,20+2*(i1-1)  ) = rv + deltax * fx 
              wallpt(id,20+2*(i1-1)+1) = zv 
            ENDIF
          ENDDO
        ENDDO      
...
c        deltax2 =  0.068
     ...
     .            zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
                fx = 1.0 - (rv - x1) / (x2 - x1)
                fy =       (zv - y1) / (y2 - y1)
c                deltax = (1.0 - fy) * deltax1 + fy * deltax2
                rvertp(i1,id) = rv + deltax * fx 
                zvertp(i1,id) = zv 
              ENDIF
            ENDDO
...
c              deltax = (1.0 - fy) * deltax1 + fy * deltax2
     ...
     .        zv.GT.MIN(y1,y2).AND.zv.LE.MAX(y1,y2)) THEN
      ...
      ELSEIF (mode.GE.2.AND.mode.LE.7) THEN
