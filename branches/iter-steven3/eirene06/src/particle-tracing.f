...
C
      IF (ITIME.EQ.1) THEN
        ...
        IF (NLRAD) THEN
c          WRITE(0,*) 'DEBUG: CALLING TIMER A'
          CALL TIMER(TS)
c          WRITE(0,*) 'DEBUG: RETURN FROM TIMER A'
C
...
C
380   CONTINUE
c      WRITE(0,*) 'DEBUG: LEV.EQ.5 4.00 at 380'
C
C   REFLECTION FROM  SURFACE
...
C
      IF (ITIME.EQ.1) THEN
        IF (NLRAD) THEN
c          WRITE(0,*) 'DEBUG: CALLING TIMER B'
          CALL TIMER(TS)
c          WRITE(0,*) 'DEBUG: RETURN FROM TIMER B'
C
          T=TS/TL-1.0_DP
c          WRITE(0,*) 'DEBUG: TIMER B 1.00',t
...
C
        ...
        ENDIF
C
c        WRITE(0,*) 'DEBUG: TIMER B 3.00',zdt1
        IF (ZDT1.LE.0.D0) GOTO 990
C
...
213     CONTINUE   ! EXIT FROM NCOU LOOP DUE TO COLLISION AT ICOU=JJ
        NCOU=JJ
      ENDIF
c      WRITE(0,*) 'DEBUG: CHECK EVENT 1.00',nlpr,zint1,zlog
C
C  CHECK FOR EVENT
C
      IF (NLPR) THEN
...
c        WRITE(0,*) 'DEBUG: CHECK EVENT OFF TO 220'
        GO TO 220
      ENDIF
C
215   CONTINUE
...
C  UPDATE CONTRIBUTION TO VOLUME AVERAGED ESTIMATORS
C
c      WRITE(0,*) 'DEBUG: VOLUME TALLY 1.00',iupdte,ityp
      IF (IUPDTE.GE.1) THEN
        IF (ITYP.EQ.1) CALL UPDATM (XSTOR2,XSTORV2,IFLAG)
        IF (ITYP.EQ.2) CALL UPDMOL (XSTOR2,XSTORV2,IFLAG)
        IF (ITYP.EQ.0) CALL UPDPHOT(XSTOR2,XSTORV2,IFLAG)
...
C  STOP TRACK ?
C
c      WRITE(0,*) 'DEBUG: STOP TRACK? 1.00',isrfcl
      IF (ISRFCL.EQ.1) CALL ADDCOL (XLI,YLI,ZLI,SG,*104,*380)
      IF (ISRFCL.EQ.2) CALL TIMCOL (AX(2),         *104,*800)
      IF (ISRFCL.EQ.3) CALL TORCOL (               *104)
C
...
C
220   CONTINUE
c      WRITE(0,*) 'DEBUG: 220 1.00'
      DIST=CLPD(NCOU)
      CLPD(NCOU)=(ZLOG-ZINT2)*ZMFP
      ZTC=ZT+CLPD(NCOU)
      IFLAG=4
c      WRITE(0,*) 'DEBUG: 220 2.00',iupdte
      IF (IUPDTE.GE.1) THEN
        IF (ITYP.EQ.1) CALL UPDATM (XSTOR2,XSTORV2,IFLAG)
        IF (ITYP.EQ.2) CALL UPDMOL (XSTOR2,XSTORV2,IFLAG)
        IF (ITYP.EQ.0) CALL UPDPHOT(XSTOR2,XSTORV2,IFLAG)
...
C  PARTICLE TRACING
C
c      WRITE(0,*) 'DEBUG: 230 2.00',nltest
      IF (NLTEST) CALL CLLTST(*997)
C
...
C
      IF (COLTYP.EQ.2.) GOTO 700
C
c      WRITE(0,*) 'DEBUG: 230 OFF TO 100'
      GOTO 100
C
C  SIMULATION OF COLLISION EVENT FINISHED
C
...
104       CONTINUE
c          WRITE(0,*) 'DEBUG: LEV.EQ.5 3.00 at 104'
          IMOL=NMOLI
103       CONTINUE
C  WEIGHT CORRECTION
          IF (NSPEZ(ISTRA).LT.0) THEN
...
!pb     .                          PTRIY(IS,ITRI)*BYIN(ITRI))/BABS)
     ...
     .                  /2._DP
            ...
            ELSEIF (LEVGEO.EQ.5) THEN
              K=0
              RRSTEP(ISTEP,1) = 0._DP
c              WRITE(0,*) 'DEBUG: SAMSRF 1.00',ntet
              DO ITET=1,NTET
                DO IS=1,4
