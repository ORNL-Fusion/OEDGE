...
c
c     jdemod - presumably by this point the grid is loaded and 
c              appropriate values have been assigned to psitarg
c            - for all cases except the JET grid now assign
c              psitarg to psifl (the psi values over the whole grid)
c            - only do this if psifl is not loaded already  
c
      if (cgridopt.ne.0.and.psifl(1,irsep).eq.0.0) then 
         do ir = 1,nrs
            write(6,'(a,2(1x,g12.5))') 'PSITARG->PSIFL:',
     >                       psitarg(ir,1),psitarg(ir,2)
            do ik = 1,nks(ir)
               if (ik.lt.nks(ir)/2) then 
                  psifl(ik,ir) = psitarg(ir,2)
               else
                  psifl(ik,ir) = psitarg(ir,1)
               endif
            end do
         end do
      endif
...
c     TO COREDAT ARRAY
c
c      write(0,*) 'process core profiles:'
c
...
c
      call pr_trace('TAU','BEFORE DOTEMP')
...
C-----------------------------------------------------------------------
C
      call pr_trace('TAU','BEFORE CALCWP')
...
c
      call pr_trace('TAU','BEFORE WRTDIVGRID')
      call wrtdivgrid
C
C-----------------------------------------------------------------------
c
...
c
      call pr_trace('TAU','BEFORE WRTDIVBG')
      call wrtdivbg
c
c      endif
c
...
c
      ...
      call pr_trace('TAU','BEFORE KES/KVHS')
...
C-----------------------------------------------------------------------
C
      call pr_trace('TAU','BEFORE KFEGS/KFIGS')
...
C-----------------------------------------------------------------------
c
      call pr_trace('TAU','BEFORE FGRADMOD')
...
c-----------------------------------------------------------------------
c
      call pr_trace('TAU','BEFORE VBGRAD')
...
c-----------------------------------------------------------------------
c
      call pr_trace('TAU','BEFORE GRADSCALELENGTHCHECK')
...
C-----------------------------------------------------------------------
C
      call pr_trace('TAU','BEFORE DIFFUSION/PINCH')
...
c slmod end
c
      call pr_trace('TAU','BEFORE KPLOS')
C
C-----------------------------------------------------------------------
...
C
      call pr_trace('TAU','BEFORE IZTAU')
c slmod begin
...
C
      call pr_trace('TAU','BEFORE CXREC')
...
C
      call pr_trace('TAU','BEFORE KPCHS/KPRCS')
...
c
      call pr_trace('TAU','BEFORE NEUT2D')
C-----------------------------------------------------------------------
c
c     NEUT2D_OPT =1
c
...
C-----------------------------------------------------------------------
c
      call pr_trace('TAU','BEFORE CALC_MPS')
...
c----------------------------------------------------------------------- 
c
      call pr_trace('TAU','BEFORE SETUP_FP')
...
C-----------------------------------------------------------------------
c
      call pr_trace('TAU','BEFORE ASSIGN_WALL_PLASMA')
...
c-----------------------------------------------------------------------
c
      call pr_trace('TAU','BEFORE CALCPOTENTIAL2')
      call CalcPotential2
c
C
C-----------------------------------------------------------------------
...
C925  CONTINUE
C
      call pr_trace('TAU','BEFORE PRINT OUTS')
...
c
      ...
      !integer, allocatable :: NJ(MAXNRS),NI(MAXNKS)
      !INTEGER,allocatable :: ITAG(MAXNKS*MAXNRS,5),ITAGDV(MAXNKS*MAXNRS)
      !INTEGER,allocatable ::  KORX(MAXNKS,MAXNRS),KORYE(MAXNRS,MAXNKS)

      INTEGER ISEP,NINOMP,NINOWA,IREF,NZ,NZMAX,K0,IK0,IKMID
      INTEGER IND,IW,in
      !REAL,allocatable ::   DUMMY(MAXNKS*MAXNRS,11)

      real    HMASS,ZMASS,S,DELTAL,P,FACT
      REAL    sPERP,BEST,DSQ,R,Z,THIN,THOUT,DELTAR,DELTAZ
      REAL    MU,SMAX,SMID,SDIFF
      real    qovrt
      real    rtmp,ztmp
      ...
      INTEGER,allocatable :: ITAG(:,:),ITAGDV(:)
      INTEGER,allocatable ::  KORX(:,:),KORYE(:,:)
...
c
      call allocate_array(nj,maxnrs,'NJ',ierr)
      call allocate_array(ni,maxnks,'NI',ierr)
      call allocate_array(itag,maxnks*maxnrs,5,'ITAG',ierr)
      call allocate_array(itagdv,maxnks*maxnrs,'ITAGDV',ierr)
      call allocate_array(korx,maxnks,maxnrs,'KORX',ierr)
      call allocate_array(korye,maxnks,maxnrs,'KORYE',ierr)
      call allocate_array(dummy,maxnks*maxnrs,11,'DUMMY',ierr)
c
C-----------------------------------------------------------------------
...
c
      ...
      if (allocated(itagdv)) deallocate(itagdv)
      if (allocated(korx)) deallocate(korx)
      if (allocated(korye)) deallocate(korye)
...
C
c     deallocate local arrays in error case
c
      if (allocated(nj)) deallocate(nj)
      if (allocated(ni)) deallocate(ni)
      if (allocated(itag)) deallocate(itag)
...
c
c     Finish off PSI information at end of grid read
c
c     IF PSIFL data has been loaded then overwrite any PSITARG data read
c      
...
c
      ...
      integer ik,ir,id
c
c     temporary variable for storing gradients
c     
...
c
c     Allocate array for gradients
c     
      allocate(grad_para(maxnks,maxnrs))
c
c     gradne - parallel density gradient
...
c
c     gradTe - parallel electron temperature gradient
c            - needs to be calculated. 
c
      grad_para = 0.0
...
c
c     gradTi - parallel ion temperature gradient
c            - needs to be calculated. 
c
      grad_para = 0.0
...
c
      write(of,'(/)')
c
c     Close the file so that all the buffers are cleared and written
c
      close(of)
c
      return
...
c
c     jdemod - write the psin data for each cell 
c
      write (of,10) 'PSIFL:'
      write (of,500) ((psifl(ik,ir),ik=1,nks(ir)),ir=1,nrs)
c
...
c
c     jdemod - Write out wall data - the most information is in the wallpt array  - try writing just rves,zves for now
c
      write(of,'(a,2(1x,i8))') 'WALLS:',wallpts,nwall_data
      do iw = 1,wallpts
         write(of,500) (wallpt(iw,in),in=1,nwall_data)
      end do
