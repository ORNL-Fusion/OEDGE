      subroutine setup_hydkin_reactions(hydkin_default, cadapt)
     ...
     .            escd3, e_el, e_k
      ...
      OPEN (UNIT=29,FILE=FILENAME,ACCESS='SEQUENTIAL',
     .      FORM='FORMATTED')
      READ (29,*)
      READ (29,*) CHR,n_reac
      READ (29,*) CHR,n_spec
     ...
     .                   (IEIGEN(1:N_SPEC) == 0))
      ...
      if (ladapt) then
        ...
        if (ic == 0) then
          ...
          call replace_string(pchr,cor,crep)
        end if
      end if
...
!  find type of species
      do isp = 1, n_spec
        ...
        call remove_char (hydspec(isp),species(isp)%name,'_^')

        ll=len_trim(hydspec(isp))

        if (ladapt) then
          oriname = hydspec(isp)
          iori = find_element(oriname(1:2))
          call replace_string(hydspec(isp),cor,crep)
          call replace_string(species(isp)%name,cor,crep)
        end if


!  check periodic table of elements
        iel = find_element(species(isp)%name(1:2))
...
!  now find the ingredients
          if (ladapt) 
     .      call find_ingred (oriname, species(isp)%mass_db,
     .                        species(isp)%ncharge, species(isp)%nprt) 

          call find_ingred (hydspec(isp), species(isp)%masse,
     .                      species(isp)%ncharge, species(isp)%nprt)
...
!pb      il = nreac_lines - n_reac
      ...
      reac_loop: do irc=1, n_reac
        ...
        call read_hydkin (ir,dbfname(ifile),h123,reac,crc,rmn,rmx,
     .                    e_el,e_k,.true.)
...
!  setup reaction specifictions for the reacting species
        ...
        if (ladapt) then
          call replace_string(rstring,cor,crep)
        ...
        call split (left,'&',nlparts,clparts)
        call split (right,'&',nrparts,crparts)
...
!PB  NOTHING TO BE DONE
     ...
     .  WRITE (27,'(6es12.4)') VL0,VL1,VL2,VL3,VL4,VL5
      ...
      subroutine remove_char (inchar, outchar ,remchar)
      ...
      subroutine replace_string (inchar,rem,rep)
      ...
      subroutine find_ingred (name,mass,icharge,ipart)
      ...
      return
      end subroutine find_ingred 

      
      subroutine split(string, splchr, nparts, parts)
      ...
      end subroutine setup_hydkin_reactions
