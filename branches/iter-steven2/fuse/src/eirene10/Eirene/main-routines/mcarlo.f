...
C
      IF ((NSTEFF > 0) .AND. (NPRS.GT.nsteff)) THEN
        if (my_pe == 0) CALL PEDIST(XTIM,XX1)
        call broad_pedist(xtim)
        if (.not.nlident) then
          do istra=1,nstrai
...
C
        IF (LALGV) THEN
          DO 830 IALV=1,NALVI
            ...
            CALL INTTAL (DUMMY,VOLTAL,1,1,NSBOX_TAL,ALGVI(IALV,ISTRA),
...
C
          DO 1571 IALV=1,NALVI
            DUMMY(1:NSBOX_TAL) = ALGV(IALV,1:NSBOX_TAL)
            CALL INTTAL (DUMMY,VOLTAL,1,1,NSBOX_TAL,ALGVI(IALV,0),
     .                   NR1TAL,NP2TAL,NT3TAL,NBMLT)
            ALGV(IALV,1:NSBOX_TAL) = DUMMY(1:NSBOX_TAL)
1571      CONTINUE
C
