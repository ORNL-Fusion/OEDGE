...
512     CONTINUE
        ...
        CALL GRDRW (REAL(XN0+7./FX,KIND(1.E0)),REAL(YNP,KIND(1.E0)))
        YNP=YNP-0.75/FY 
...
C
     ...
     .           VELX,VELY,VELZ,VEL,E0)
        ENDIF
        CALL MASR2 ('WEIGHT,TIME   ',WEIGHT,TIME)
...
C  CALLED FROM DIAGNO, WITH ISTRA=0?
     ...
     .      (ISYM.EQ.1.AND.NLSRF(ISTR))) THEN
          IF (NLSRFX) THEN
            CALL MASJ1 ('MRSURF  ',MRSURF)
          ELSEIF (NLSRFY) THEN
            CALL MASJ1 ('MPSURF  ',MPSURF)
          ELSEIF (NLSRFZ) THEN
            CALL MASJ1 ('MTSURF  ',MTSURF)
          ELSEIF (NLSRFA) THEN
            CALL MASJ1 ('MASURF  ',MASURF)
          ENDIF
