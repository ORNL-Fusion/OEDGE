...
20      CONTINUE
        ...
        IF (TRCINT) WRITE (6,*) 'NDXA,NDYA,NDZA ',NDXA,NDYA,NDZA
...
C  NTIN,NTEN: SOURCE RANGE FROM GRIDPOINT NTIN TO GRIDPOINT NTEN
     ...
     .                              NSPZE(IT,IPRT)
            ...
            ENDIF
            IF (TRCINT)
     .        WRITE (6,'(12I6)') IT,NIXY(IT,IPRT),N1IN,N1EN,
     .                              N2IN,N2EN,N3IN,N3EN,
     .                              NIFLG(IT,IPRT),
     .                              NPTC(IT,IPRT),NSPZI(IT,IPRT),
     .                              NSPZE(IT,IPRT)
...
C
      IF (NDYA.NE.NR1ST.OR.NDYA.GT.NDY) THEN
        ...
        WRITE (6,*) ' NDYA MUST BE = NR1ST AND <= NDY'
        WRITE (6,*) ' NDYA,NR1ST,NDY = ',NDYA,NR1ST,NDY
        CALL EXIT
      ELSEIF (NDXA.NE.NP2ND.OR.NDXA.GT.NDX) THEN
        WRITE (6,*) ' PARAMETER ERROR DETECTED IN INTFCE '
        WRITE (6,*) ' NDXA MUST BE = NP2ND-1 AND <= NDX'
        WRITE (6,*) ' NDXA,NP2ND,NDX = ',NDXA,NP2ND,NDX
        CALL EXIT
      ELSEIF (NDZA.NE.NT3RD) THEN
        WRITE (6,*) ' PARAMETER ERROR DETECTED IN INTFCE '
        WRITE (6,*) ' NDZA MUST BE = NR3RD'
        WRITE (6,*) ' NDZA,NT3RD = ',NDZA,NT3RD
        CALL EXIT
      ENDIF
C
...
C  SET UP TETRAHEDRONS
     ...
     .              (ntet >= ntfalse(itfal))) then
                  write (6,*) ' itet = ', ntfalse(itfal)
                  write (6,*) ' ir, ip, it ', ir, ip, it
                  write (6,*) ' ic1 = ',ic1
                  write (6,*) ' ic2 = ',ic2
                  write (6,*) ' ic3 = ',ic3
                  write (6,*) ' ic4 = ',ic4
      ...
      do ists=1,nstsi
        ...
        if (iliin(msurf) == 4) then
          if (idez(iliin(msurf),2,2) .ne. 0) then
            write (6,*) ' conflict in surface definition '
            write (6,*) ' program stopped '
            call exit  
          else
            iliin(msurf)=iliin(msurf)+10*mper
            write(6,*) ' msurf ',msurf,' iliin changed to ',iliin(msurf)
          endif
        endif
      end do
      DEALLOCATE (IRDSTP)
