...
c
c      real ctime
c      real etime,rdum1,val(2)
c
c     Replace MCLOCK with call to ETIME for COMPAQ FORTRAN
c
c     Just use elapsed time for now - it isn't accurate - but 
c     it does reflect the total time ... it looks like COMPAQ 
c     fortran will not include the execution time of child processes 
c     when calculating elapsed CPU time. (Or I haven't found the 
c     correct routine. 
c
c      call cpu_time(ctime)
c
c      rdum1=etime(val)
c      za02as = val(1)
c
...
c     Signal trapping commented out for COMPAQ - call to signal causes
c     a segmentation fault. 
c    
c      call signal(%VAL(SIGUSR1),killdiv)
c
      return
      end
