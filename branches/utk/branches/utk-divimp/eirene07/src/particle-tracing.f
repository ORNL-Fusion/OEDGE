...
C  SPATIAL RESOLUTION ON NON DEFAULT STANDARD SURFACE?
      ...
      ELSE
        ...
        RETURN
      ENDIF
c      WRITE(0,*) 'DEBUG: MSURF=',msurf,nlim,nsts,msurfg
c      WRITE(0,*) 'DEBUG:      =',ipolgn,mrsurf
C
...
c      WRITE(0,*) 'DEBUG: LOCATE',istra,nstrai
...
C
      IFPLS=0
      DO 2 IPLS=1,NPLSI
        WRITE(6,*) 'DEBUG: REC A',IPLS,LGPRC(IPLS,0),.NOT.LPLSSR(IPLS)
        IF (LGPRC(IPLS,0).EQ.0) GOTO 2
        IF (.NOT.LPLSSR(IPLS)) GOTO 2
        IFPLS=IFPLS+1
        IFREC(IPLS)=IFPLS
        WRITE(6,*) 'DEBUG: REC B',NPRCI(IPLS)
        DO 3 IIRC=1,NPRCI(IPLS)
          IRRC=LGPRC(IPLS,IIRC)
          KK=NREARC(IRRC)
          ICCT=NREACT(KK)
        WRITE(6,*) 'DEBUG: REC C',NSBOX
          DO 3 J=1,NSBOX
            ADD=0.
...
C  EXCLUDE IPLS-VACUUM CELLS
            ...
            END IF
            IF (J.EQ.1) THEN
               WRITE(6,*) 'DEBUG: REC D',ICCT
               WRITE(6,*) 'DEBUG: REC D',NSTGRD(J),.NOT.LGVAC(J,IPLS)
               WRITE(6,*) 'DEBUG: REC D',IPLS,J
               WRITE(6,*) 'DEBUG: REC D',DIIN(IPLS,J)
               WRITE(6,*) 'DEBUG: REC D',NSTORDR,NRAD
            ENDIF
            IF (ICCT > 0) 
