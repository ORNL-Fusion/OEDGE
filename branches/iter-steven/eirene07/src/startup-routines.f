...
C
     ...
     .           IDIREC, ISTCHR, JFEXMN, JFEXMX, ITOK, IER
      INTEGER, SAVE :: NZADD
      INTEGER, EXTERNAL :: IDEZ
      LOGICAL :: LHELP(NLIMPS)
      LOGICAL :: LRPS3D, LRPSCN, LHYDDEF
      CHARACTER(10) :: CDATE, CTIME
      CHARACTER(12) :: CHR, HYDKIN_DEFAULT
...
109   READ (IUNIN,'(A72)') ZEILE
     ...
     .                    NSMSTRA,NSTORAM,NGSTAL,NRTAL1,NREAC_ADD
        READ (IUNIN,6665) NLSCL,NLTEST,NLANA,NLDRFT,NLCRR,
     .                    NLERG,LIDENT,LHABER,LRCHRD,LTSTV
      ELSE
...
!  READ REAC
        IF (INDEX(FILNAM,'CONST') == 0) THEN
          CALL READ_TOKEN(ZEILE(IEND:),' ',REAC2,ITOK,IER)
          IEND = IEND + ITOK
          IF (IER > 0) THEN
            WRITE (iunout,*) 
...
!  CHECK LINE FOR FT-FLAG (IFTFLG)
          ...
          IF (ITOK /= 0) THEN
            ...
            IEND = IEND+ITOK+2
            CALL READ_TOKEN(ZEILE(IEND:),' ',CHR,ITOK,IER)
            IEND = IEND + ITOK
          END IF
        END IF
!  READ CRC
        CALL READ_TOKEN(ZEILE(IEND:),' ',CRC,ITOK,IER)
        IEND = IEND + ITOK
        IF (IER > 0) THEN
...
c slmod begin
      integer :: len_in, len_out, ianf, iend, len_tok
c
c      integer :: len_in, len_out, ianf, iend, len_tok, itok
c slmod end
