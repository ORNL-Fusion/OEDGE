...
C  OUTPUT:
     ...
     .          T133, T125, T126, T131, T154, T155, T156, T148, T144,
...
C
c slmod begin 
c...  I am not sure if this will break if X,YVERT are not allocated?
      SUBROUTINE ARPOLY(X,Y,XV,YV,M3,M2,N,M1,MI,ME,AREA2,XCOM,YCOM,
     .                  OPTGRD)
c
...
C
     ...
     .          XCD1, YCD1, XCD2, YCD2
      INTEGER :: I, J, IP1, JP1, NM1, MEM1, IN
c slmod begin
      INTEGER,INTENT(IN) ::  M2, M3, OPTGRD
      REAL(DP),INTENT(IN) ::  XV(M3,M2,2), YV(M3,M2,2)
c...  temp:
      REAL*8 zill      
c slmod end
...
15    LEARCA=I-1
      RETURN
C
20    WRITE (6,*) 'X OUT OF RANGE IN LEARCA'
      WRITE (6,*)  X,NNN,R(NS,NNN)
      WRITE (6,*) 'LEARCA= ',NNN,' RETURNED TO SUBR. ',TEXT
C
      RETURN
      END
c === ROUTINE: leer
      ...
      IF (IFILE.GT.0) THEN
        DO IST=1,NSTREAM
          IF (IFILE.EQ.ISTREAM(IST)) THEN
            WRITE (6,*) 'ERROR IN INPUT BLOCK 11, NTLVFL '
            WRITE (6,*) 'STREAM NO. ',IFILE,' IS NOT AVAILABLE'
            GOTO 11111
          ENDIF
        ENDDO
        WRITE (IFILE,*) TL
        ...
        DO IRAD=1,NTT,5
          WRITE (IFILE,*) (PROF(IR),IR=IRAD,IRAD+4)
        ENDDO
      ENDIF
...
C
      ...
      CHARACTER(72), INTENT(OUT) :: T2, T3
      REAL(DP), INTENT(OUT) :: PROF(*)
      INTEGER, INTENT(IN) :: IFILE
      INTEGER, INTENT(OUT) :: NR, NP, NT, NB, NTT, IFLAG
      INTEGER, PARAMETER :: NSTREAM=15
      REAL(DP) :: H(6)
      INTEGER :: K(6), ISTREAM(NSTREAM)
...
C     DATA TL/72*'='/
      ...
      IF (IFILE.GT.0) THEN
        DO IST=1,NSTREAM
          IF (IFILE.EQ.ISTREAM(IST)) THEN
            WRITE (6,*) 'ERROR IN INPUT BLOCK 5, INDPRO '
            WRITE (6,*) 'STREAM NO. ',IFILE,' IS NOT AVAILABLE'
            GOTO 11111
          ENDIF
        ENDDO
...
c
      ...
      USE PRECISION
      implicit none
      real(dp) x05baf,start,reset_second,second_own
      save start
      data start /0.0/
c x05baf is a NAG routine
      SECOND_OWN=x05baf()-start
CNAG  call cpu_time(time)
CNAG  second_own=time-start
      RETURN
C
      ENTRY RESET_SECOND()
      start=x05baf()
CNAG  call cpu_time(start)
      reset_second=start
      return
      END
C
...
C
C
      SUBROUTINE WRSTRT(IG,NSTRAI,IESTM1,IESTM2,TALLYV,TALLYS,
     .                  ISDVI1,STAT1,ISDVI2,STAT2,
     .                  ISDVC1,SIGC,ISDVC2,SIGCS,
     .                  IBGKI,SIG_BGK,JBGKI,SIGS_BGK,
     .                  ICOPI,SIG_COP,JCOPI,SIGS_COP,TRCFLE)
...
C
      ...
      IMAX=IMAX11+IMAX12+IMAX2+IMAX31+IMAX32+IMAX41+IMAX42
      ISTRA=IG
      IRC=ISTRA*IMAX+1
...
C
6     CONTINUE
      IF (TRCFLE) WRITE (6,*) 'SUM STATIS_BGK'
      IRC=IRC+1
      JINI=1
...
C
8     CONTINUE
      IF (TRCFLE) WRITE (6,*) 'SUM STATIS_COP'
      IRC=IRC+1
      JINI=1
...
C
      RETURN
C
      ENTRY RSTRT(IG,NSTRAI,IESTM1,IESTM2,TALLYV,TALLYS,
     .            ISDVI1,STAT1,ISDVI2,STAT2,
     .            ISDVC1,SIGC,ISDVC2,SIGCS,
     .            IBGKI,SIG_BGK,JBGKI,SIGS_BGK,
     .            ICOPI,SIG_COP,JCOPI,SIGS_COP,TRCFLE)
