...
C
     ...
     .                 HEAD5, HEAD6, HEAD7, HEAD8, HEAD9, TXHEAD
C
C
      SAVE
      DATA IFIRST/0/
...
C  NOTHING TO BE DONE
     ...
     .               NSBGK,SIGMA_BGK,NBGV_STAT,SGMS_COP,
...
C
     ...
     .           '
      HEAD9='ENERGY SPECTRUM OF FLUXES ON SURFACES
     .           '
C
      IALG=0
C
...
111               VECTOR(I,ICURV)=0.
                ...
                CASE DEFAULT
                  WRITE (6,*) ' WRONG TALLY NUMBER IN PLTEIR',
     .                        ' JTAL = ',JTAL
                  WRITE (6,*) ' NO PLOT PERFORMED '
                  CALL LEER(1)
                  GOTO 10000
                END SELECT
     ...
     .                        ' JTAL = ',JTAL
            ...
            ELSEIF (JTAL.GE.0) THEN
              IF (.NOT.LIVTALV(JTAL)) THEN
                WRITE (6,*) TXTTAL(1,JTAL)
                WRITE (6,*) 'TALLY SWITCHED OFF '
                WRITE (6,*) 'ALL PLOTS FOR THIS TALLY TURNED OFF '
                GOTO 10000
              END IF
              NFT=NFSTVI(ITL)
...
C
              ...
              IF (JTAL.GT.0.) THEN
                ...
                CALL FETCH_OUTAU (OUTAUI,JTAL,ISPZ,ISTRA)
                IF (OUTAUI.EQ.0.) THEN
                  IF (TRCPLT) THEN
...
150         CONTINUE
     ...
     .             N1SDVI,NRAD)
            IF (TRCPLT) THEN
              IF (IERR.GT.0) THEN
                WRITE (6,*) '2D PLOT FOR TALLY NO. ',JTAL,' ABANDONED'
                WRITE (6,*) 'ERROR CODE FROM SUBR. PLTTLY: ',IERR
                WRITE (6,*) 'XMI,XMA ',XMI,XMA
                GOTO 1000
              ENDIF
...
1222            CONTINUE
                ...
                IF (LRAPS3(IBLD)) THEN
                  TMAX=101.
                  WRITE (6,*) ' IBLD, ICURV ',IBLD, ICURV, INULL,
     .                        ' TETRAHEDRONS WITH 0 PARTICLES '
                END IF
                LSDVI(ICURV)=.FALSE.
                GOTO 1200
              ENDIF
...
C  THERE ARE NSPS BINS, AND NSPS+1 ENERGY BIN BOUNDARIES
        ...
        DO I=1,NSPS+1
          XSPEC(I,1)=SPCAN+(I-1)*ESTIML(ISPC)%PSPC%SPCDEL
