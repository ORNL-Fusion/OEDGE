...
c     being used:
     ...
     >                 '*********************************************'
                  WRITE(0,*)
     >                 '*  WARNING: CITERSOL.EQ.2 logic has changed *'
                  WRITE(0,*)
     >                 '*********************************************'
                  WRITE(0,*) 
               ENDIF
...
c     
     ...
     >           .and.(cioptf.eq.99.or.cioptg.eq.99.or.cioptg.eq.90))
     >           then
               call calcefb2
            endif
c     
...
c     
         ...
         call pr_trace('BGPLASMA','BASIC PLASMA LOADED')
...
c     CALL SaveSolution
         IF (rel_opt.EQ.1.AND.
     .        (rel_iter.EQ.rel_niter.OR.rel_step.EQ.0))
     .        CALL SaveSolution
...
C     
                     CHISQ1(IITERSOL-1)=CHISQ1(IITERSOL-1)
     >                    +(KTEBS(IK,IR)-
     >                    OKTEBS(IK,IR))**2
                     CHISQ2(IITERSOL-1)=CHISQ2(IITERSOL-1)
     >                    +(KTIBS(IK,IR)-
     >                    OKTIBS(IK,IR))**2
c     
...
c...  Change output option:
         IF (outmode.GE.2) THEN
            WRITE(67,'(A,I4,A)') 'ITERATION ',rel_count,
     .           ' (after update)'
            WRITE(67,*)
...
c     frac = rel_bound1 + frac * (rel_bound2 - rel_bound1) 
     ...
     .           tcd.GE.0.0.AND.tcd.LT.1.0) THEN
               ...
               IF     (osms28(i1,1).EQ.REAL(type)) THEN
                  ...
                  IF (n.GT.MAXVAL) CALL ER('MapParameters',
...
c...  Quick check on if all entries were valid:
     ...
     .                 CALL ER('PrescribeFlow',
     .                 's-value out of range',*99)
                  IF (v(i1).LT.-1.0.OR.v(i1).GT.1.0) 
     .                 CALL ER('PrescribeFliow',
     .                 'v-value out of range',*99)
