...
c
      ...
      INTEGER   fp,ik,ir,ik1,ir1,ir_shift
...
c        CASE (3)
c          READ(fp,*) 
c          WRITE(buffer,'(1024X)')
c          buffer(1:1) = '*'
c          DO WHILE(buffer(1:1).EQ.'*')
...
c       ----------------------------------------------------------------
 ...
 30       CONTINUE
c       ----------------------------------------------------------------
        CASE (4)  ! .sup file generated by OSM, so there needs to be an index correction
          DO ir = 1, nrs
            IF (idring(ir).EQ.BOUNDARY) CYCLE
            ir_shift = 1
            IF (ir.GT.irtrap) ir_shift = 3
            DO ik = 1, nks(ir)
              IF (ir.LT.irsep.AND.ik.EQ.nks(ir)) CYCLE
              READ(fp,'(A1024)',END=35) buffer
c              WRITE(0,*) 'BUFFER',buffer(1:100)
              READ(buffer,*) ik1,ir1,rdum1
              IF (ik.NE.ik1.OR.ir.NE.ir1+ir_shift) 
     .          CALL ER('LoadGridData','Cell index mismatch',*99)
              IF (ik.EQ.1        ) psitarg(ir,2) = rdum1
              IF (ik.EQ.nks(ir)-1) psitarg(ir,1) = rdum1
            ENDDO
          ENDDO
 35       CONTINUE
c       ----------------------------------------------------------------
