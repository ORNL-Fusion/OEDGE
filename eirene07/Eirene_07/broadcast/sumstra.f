...
!  18.12.06  group management renewed
      ...
      integer :: ngrpmem, iold, istr, i, color
      integer :: npenum(nstra)
C

      ngrpmem = 0   
      ngrpmem = count(npts(1:nstrai) > 0)
      npenum(1:ngrpmem) = pack(npesta(1:nstrai),npts(1:nstrai) > 0)

      write (0,*) my_pe,' in sumstra '
      call mpi_barrier(mpi_comm_world,ier)

      if (my_pe == 0) then
        write (0,*) ' no of group members ',ngrpmem
        write (0,*) ' group members ',npenum(1:ngrpmem)
      end if
      ...
      call mpi_comm_split (mpi_comm_world,color,my_pe,icomgrp,ier)
      
      IF (color == 1) THEN
        CALL MPI_COMM_RANK(ICOMGRP,MY_GRP_RNK,ier)
        write (0,*) my_pe,' in sumstra if block ',MY_GRP_RNK
...
C
     ...
     .                  mpi_logical,mpi_LOR,0,icomgrp,ier1)
      ...
      write (0,*) my_pe,' at end of sumstra '
      RETURN
      END
