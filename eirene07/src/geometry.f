...
C
      ...
      END
C ===== SOURCE: df_dxyz.f
      subroutine df_dxyz (fecken, icell, x, y, z, dfdx, dfdy, dfdz) 
     ...
     .            deter4x4, deti, r, s, t, u, dxdr, dxds, dydr, dyds,
     .            drdx, dsdx, drdy, dsdy
     ...
     .            dndr(4), dnds(4), j(2,2), jm1(2,2)
      ...
      if ((levgeo == 2) .or. (levgeo == 3)) then
        ...
        call ncelln(icell,ir,ip,it,ia,ib,nr1st,np2nd,nt3rd,nbmlt,
     .              nlrad,nlpol,nltor)
        ...
        call xyz_to_rst(icell, x1, y1, 0._dp, x2, y2, 0._dp, 
...
! partial derivatives of shape functions  
        ...
        jmt = jmt / (jt(1,1)*jt(2,2) - jt(1,2)*jt(2,1))
        ...
        jm1 = jm1 / (j(1,1)*j(2,2) - j(1,2)*j(2,1))
...
!pb         call exit_own(1)
      ...
      return
      end subroutine df_dxyz

      
C ===== SOURCE: femint.f
      function femint (fecken, icell, x, y, z) result(res)
     ...
     .      + f4 * 0.25_dp * (1._dp - r) * (1._dp + s)
        ...
        call xyz_to_rst(icell, x1, y1, 0._dp, x2, y2, 0._dp, x3,
     ...
     .                  x, y, z, r, s, t, u)
        ...
        call xyz_to_rst(icell, x1, y1, z1, x2, y2, z2, x3, y3, z3,  
     .                  x4, y4, z4, x, y, z, r, s, t, u)
...
! isoparametric quadrilateral (see chapter 23, AFEM)
        ...
        xip = 2._dp*c_xi / 
     .        (-sqrt(b_xi*b_xi - 2._dp*j1(icell)*c_xi) - b_xi)
        etap = 2._dp*c_eta / 
     .        ( sqrt(b_eta*b_eta + 2._dp*j2(icell)*c_eta) - b_eta)
