...
c      IF (NPHOTI.EQ.0.AND.NITER.GE.1.AND.IITER.LT.NITER) THEN
        WRITE(fp,*) 'CALLING MODBGK'
        CALL MODBGK
c        RETURN
      ENDIF

      IF (IITER.GE.NITER) THEN
c        WRITE(fp,*) 'CALLING OUTUSR'
c        CALL OUTUSR
      ENDIF

      IF (NPHOTI.EQ.0) RETURN
      WRITE(fp,*) 'WHOA! CALLING PHOTON CODE!'
c slmod end
c
...
c     Bulk species:
      ...
      NIPLS=1
      BULK_SOURCES(1) = .TRUE.
      IF (NPHOTI.EQ.0.AND.NITER.GE.1) NIPLS=3      ! Include D and D2 BGK bulk "ion" species only...
          ...
          DO ISRFS=1,NSRFSI(ISTRA)
            ...
            IF (INMTI(ISIDE,ITRI) == INDSRF) THEN
              ...
              IF (NSPEZ(ISTRA) <= 0) THEN
                ...
                IPLEN(ISTEP)=NPLSI
c slmod begin
c              ELSEIF (NSPEZ(ISTRA) > NPLSI) THEN
c                IPLAN(ISTEP)=1
c                IPLEN(ISTEP)=NPLSI
c slmod end
              ELSE
...
c slmod begin - debug
cc              IF (istra.EQ.2) THEN
c                WRITE(0,'(A,3I9,2F10.2,2I4)') 
c     .            '      W:',ipls,istep,kstep(istep),delr,ABS(flx),
c     .                  iplan(istep),iplen(istep)
...
c slmod end            
              DO IPLS=IPLAN(ISTEP), IPLEN(ISTEP)
                ...
                VXSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = VXIN(IPLS,ITRI) ! Approximation
                VYSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = VYIN(IPLS,ITRI)
                VZSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = VZIN(IPLS,ITRI)
c slmod begin
                IF (NSPEZ(ISTRA) <= 0.0) THEN
                  STOP 'HELIUM BUSINESS, BEWARE'
                  IF (IPLS.EQ.1) FLSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = 
     .              ABS(FLX)/DELR*0.01
                  IF (IPLS.EQ.2) FLSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = 
     .              ABS(FLX)/DELR*0.99
                  IF (IPLS.EQ.3) STOP 'PLAUSR: SOMETHINGS WRONG'
                ELSE
                  FLSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = ABS(FLX)/DELR
                ENDIF
c
c                FLSTEP(IPLS,ISTEP,KSTEP(ISTEP)) = ABS(FLX)/DELR
c slmod end
              END DO
              EXIT STRATA
            END IF
...
C ===== SOURCE: usrtrc.f
      ...
      INTEGER  IFLAG,ISYM,count,count1
      ...
      WRITE(90,'(I8,3F11.3,3I4,I4,I7,1P,1E10.2,0P,F8.4,I6,7I5,2X,5I5,
     .  1P,E10.2,0P)') 
...
c      IF (count1.EQ.1000) STOP 'HALTING CODE'
