...
!      pz =1 
!           !do pz = 1, maxpzone
!      write(6,*) 'TARGET CONDITIONS BEFORE:',pz
!      do ix = 1,nxs
!
!                !do iy = -nys,nys
!               if (xouts(ix).le.0.0) then 
!               iqx = iqxs(ix)
!               write(6,'(3i8,30(1x,g12.5))') ix,pz,iqx,
!     >            xouts(ix),
!     >            crnbs(ix,-1,pz),crnbs(ix,0,pz),crnbs(ix,1,pz),
!     >            ctembs(ix,-1,pz),ctembs(ix,0,pz),ctembs(ix,1,pz),
!     >            ctembsi(ix,-1,pz),ctembsi(ix,0,pz),ctembsi(ix,1,pz),
!     >              qrnbs(iqx,1),qrnbs(iqx,2),
!     >              qtembs(iqx,1),qtembs(iqx,2),
!     >              qtembsi(iqx,1),qtembsi(iqx,2)
!                else
!               write(6,'(3i8,30(1x,g12.5))') ix,pz,iqx,
!     >            xouts(ix),
!     >            crnbs(ix,-1,pz),crnbs(ix,0,pz),crnbs(ix,1,pz),
...
!         pz =1 
!           !do pz = 1, maxpzone
!            write(6,*) 'TARGET CONDITIONS AFTER:',pz
!            do ix = 1,nxs
!
!                !do iy = -nys,nys
!               if (xouts(ix).le.0.0) then 
