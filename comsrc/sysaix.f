C  *********************************************************************
C  *                                                                   *
C  *  SYSAIX :THIS FILE CONTAINS DUMMIES FOR ALL THE EXTERNAL ROUTINES *
...
c 
      ...
      character filename*(*)
c
c     Access environment variable for case name  
c
       integer   len1,lenstr
       external lenstr 
c
...
c
c
c      subroutine ioname(iunit,filename,itemp,ierr)
c      implicit none
c      integer iunit, itemp, ierr
...
c
c      integer   l1, l2
c      character cmd*80, cmda*40, cmdb*40, cmdc*2, cmdd*2
c      data      cmda/'ls -l fort.'/,
c     >          cmdb/' 2>/dev/null | cut -f2 -d''>''  > ./fort.'/
c
c      ierr = 0
c
c      filename = ' '
...
c
c      if (iunit.lt.1 .or. iunit.gt.99 .or.
c     >    itemp.lt.1 .or. itemp.gt.99) then
c        ierr = 1
c        return
c      endif
c
...
c
c      write(cmdc,'(i2)') iunit
c      l1 = 2 - min0(1,iunit/10)
c      write(cmdd,'(i2)') itemp
c      l2 = 2 - min0(1,itemp/10)
c      cmd = cmda(1:11)//cmdc(l1:2)//cmdb(1:39)//cmdd(l2:2)
c
c      call system(cmd)
c
...
c
c      rewind(itemp)
c      read(itemp,'(1x,a80)',end=20) filename
c      goto 10
c   20 ierr = 2
c
...
c     DEFINE the SIGUSR1 kill signal so that the
c     signal call can trap it - if it is sent
c     to the DIVIMP process. (Workstatio nonly)
c
      integer SIGUSR1
...
c      WRITE(6,*) 'WARNING: CALLING CLOCK2 - NOT READY'
