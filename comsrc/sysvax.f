...
c      WRITE (6,'('' XUFLOW: DUMMIED OUT FOR THIS APPLICATION.'')')
	write (6,*) 'XUFLOW: CALLED LIB$ESTABLISH'
      CALL  LIB$ESTABLISH(FAKESIGNAL) 
C       call cuflow
c	described in datetime.c
      RETURN
      END
C
C***********************************************************************
...
C
      ...
      res = MCLOCK()
      ZA02AS = res/100000.0
c      write(6,*) 'ZA02AS:MCLOCK:',res,za02as 
...
C
      ...
      command = 'dir'
c
      write(6,'(2a)') ' PIN COMMAND:',actpin
c
      CALL DECC$SYSTEM(%REF(ACTPIN))
      retcode = 0
C
c
      retcode2 =  SYSTEM(%REF(command))
C
      write(6,*) 'RETCODE2:',retcode2
c
...
c
c      call DECC$system(cmd)
c
c  read filename from temporary file
c
c      rewind(itemp)
c      read(itemp,'(2x,a80)',end=20) filename
c      goto 10
c   20 ierr = 2
c
...
c               system module.
C
      call DECC$signal(SIGUSR1,killdiv)
      return
      end
