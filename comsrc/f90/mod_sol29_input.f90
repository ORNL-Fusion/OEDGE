...
contains 
  ...
  subroutine sol29_unstructured_input(tag, line)
    ...
    ! Just some arbitrarily high max value. 
    sol29_machhi = 1.0e37
    ...
    if (tag(1:3).eq.'X01') then
      call readr(line, vr_gamma_loc, 0.0, sol29_machhi, &
        'SOL29: vr_gamma_loc') 
    elseif (tag(1:3).eq.'X02') then
      call readr(line, vr_gamma_scale, 0.0, sol29_machhi, &
        'SOL29: vr_gamma_scale')      
    elseif (tag(1:3).eq.'X03') then
      call readr(line, vr_gamma_a, 0.0, sol29_machhi, &
        'SOL29: vr_gamma_a')   
    elseif (tag(1:3).eq.'X04') then
      call readr(line, vr_gamma_c, 0.0, sol29_machhi, &
        'SOL29: vr_gamma_c')
    
    ! Analogous to qtim, just using a different variable.
    elseif (tag(1:3).eq.'X05') then
      call readr(line, timestep, 0.0, sol29_machhi, &
        'SOL29: timestep')
    ...
    elseif (tag(1:3).eq.'X06') then
      call readr(line, blob_ne, 0.0, sol29_machhi, &
        'SOL29: blob_ne')   
    elseif (tag(1:3).eq.'X07') then
      call readr(line, blob_te, 0.0, sol29_machhi, &
        'SOL29: blob_te') 
    elseif (tag(1:3).eq.'X08') then
      call readr(line, tau_ne, 0.0, sol29_machhi, &
        'SOL29: tau_ne')  
    elseif (tag(1:3).eq.'X09') then
      call readr(line, tau_te, 0.0, sol29_machhi, &
        'SOL29: tau_te')
        
    ! Starting target Te to calculate the parallel flows on the first 
    ! iteration.
    elseif (tag(1:3).eq.'X10') then
      call readr(line, seed_targ_te, 0.0, sol29_machhi, &
        'SOL29: seed_targ_te')
        
    ! Width of the blobs.
    elseif (tag(1:3).eq.'X11') then
      call readr(line, blob_radius, 0.0, sol29_machhi, &
        'SOL29: blob_radius')
    ...
    elseif (tag(1:3).eq.'X12') then
      call readi(line, nblobs, 0, sol29_machhi, &
        'SOL29: nblobs')  
    elseif (tag(1:3).eq.'X13') then
      call readi(line, niterations, 0, sol29_machhi, &
        'SOL29: niterations')
    ...
    elseif (tag(1:3).eq.'X17') then
      call readr(line, blob_length, 0.0, sol29_machhi, &
        'SOL29: blob_length')
    
    ! Measured blob frequency. Assumed constant everywhere.
    elseif (tag(1:3).eq.'X18') then
      call readr(line, blob_freq, 0.0, sol29_machhi, &
        'SOL29: blob_freq')
    ...
    elseif (tag(1:3).eq.'X19') then
      call readr(line, vr_offset, -sol29_machhi, sol29_machhi, &
        'SOL29: vr_offset')
        
    ! How many holes (inward moving blobs) to follow as a fraction of
    ! nblobs.
    elseif (tag(1:3).eq.'X20') then
      call readr(line, frac_holes, 0.0, sol29_machhi, &
        'SOL29: frac_holes')
    
    ! Similar to the above, just for the holes.
    elseif (tag(1:3).eq.'X21') then
      call readr(line, hole_ne, -1.0, sol29_machhi, &
        'SOL29: hole_ne')   
    elseif (tag(1:3).eq.'X22') then
      call readr(line, hole_te, -1.0, sol29_machhi, &
        'SOL29: hole_te') 
    elseif (tag(1:3).eq.'X23') then
      call readr(line, hole_tau_ne, -1.0, sol29_machhi, &
        'SOL29: hole_tau_ne')  
    elseif (tag(1:3).eq.'X24') then
      call readr(line, hole_tau_te, -1.0, sol29_machhi, &
        'SOL29: hole_tau_te')
    ...
    elseif (tag(1:3).eq.'X25') then
      call readr(line, tau_rad_start, 0.0, sol29_machhi, &
        'SOL29: tau_rad_start')
    ...
    elseif (tag(1:3).eq.'X27') then
      call readr(line, vr_gauss_loc, 0.0, sol29_machhi, &
        'SOL29: vr_gauss_loc')
    elseif (tag(1:3).eq.'X28') then
      call readr(line, vr_gauss_scale, 0.0, sol29_machhi, &
        'SOL29: vr_gauss_loc')
