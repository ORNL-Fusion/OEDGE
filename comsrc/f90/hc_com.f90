...
Module ComHC
  ...
  !     Input arrays.
    Real, allocatable :: HC_Reflection_Coefs(:) ! Probability of reflection. Can be individually set or set all to same preset value with H28.
    Real, allocatable :: HC_Sticking_Coefs(:)	! Probability of sticking (equals 1.0 - Prob. of sputtering). Can be individually set or set all to same preset value with H36.
...
contains
  ...
  end subroutine initialize_comHC

  subroutine allocate_comhc
    use allocate_arrays
    implicit none
    integer :: ierr

    call allocate_array(HC_Reflection_Coefs, maxnds,'HC_Reflection_Coefs',ierr)
    call allocate_array(HC_Sticking_Coefs, maxnds,'HC_Sticking_Coefs',ierr)

  end subroutine allocate_comhc


  subroutine deallocate_comhc
    implicit none
    if (allocated(HC_Reflection_Coefs)) deallocate(HC_Reflection_Coefs)
    if (allocated(HC_Sticking_Coefs)) deallocate(HC_Sticking_Coefs)

  end subroutine deallocate_comhc
