     ...
     .                 MAX (0.0, PRADIS(3,IZ+1,1,IK)*1.0E6)
          ...
          ELSE
            ...
            IF     (mode.EQ.1) THEN
              ...
              ENDDO
            ELSEIF (mode.EQ.2) THEN
              stop 'betrayal, should not be here'
              DO IK = 1, NKS(IR)
                DO IZ = 1, iz_max
...
c
      ...
      DO i1 = 2, 1, -1
        ...
        WRITE(fp,*)
        WRITE(fp,'(1X,A4,A5,2A9,2X,2A10,2A7,2X,A7,3A10)')
     .    'ir','sol','psin','rho','jsat','ne','Te','Ti','gamma',
     .    'ísat','P Flux','H Flux'
        sum_target = 0.0
        DO ring = 1, nrs-irsep+1
          IF (ring.LT.nrs-irtrap+1) THEN
          ...
          qperp = GetHeatFluxPerp(target,ir)
          WRITE(fp,'(1X,I4,2F8.4,'//
     .              '1P,2E10.2,0P,2F8.2,2X,'//
     .              '2F10.2,2X,'//
     .              '3F8.3,2X,A)')
     .      ir,psitarg(ir,1),rho(ir,CELL1),
     .      jsat,knds(id),kteds(id),ktids(id),
     .      gamma,qperp/1.E6,
     .      brat,costet(id),180.8*ASIN(brat*costet(id))/3.141592,
     .      irtag(ir)
c          WRITE(fp,'(1X,I4,I5,2F9.5,2X,'//
c     .              '1P,2E10.2,0P,2F7.2,2X,'//
c     .             'F7.2,1P,2E10.2,0P,2X,A)')
c     .      ir,model,psitarg(ir,1),rho(ir,CELL1),
c     .      jsat,knds(id),kteds(id),ktids(id),
c     .      gamma,isat,qperp,
c     .      dds2(id),
c     .      irtag(ir)


          sum_target = sum_target + qperp
        ENDDO
