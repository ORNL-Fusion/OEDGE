...
C
      ...
      WRITE(0,*) 'DEBUG: UPDMOL 1.00'
C
      WV=WEIGHT/VEL
      NPBGK=NPBGKM(IMOL)
C
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',NADVI,NCPVI,NPBGK,LBGKV
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',
     .  SIZE(xstor2,1),SIZE(xstor2,2),SIZE(xstor2,3)
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',
     .  mstor1,mstor2,n2nd+n3rd
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',
     .  SIZE(xstorv2,1),SIZE(xstorv2,2)
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',
     .  nstorv,n2nd+n3rd
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',wv
      WRITE(0,*) 'DEBUG: UPDMOL 2.00',iflag
      IF (NADVI.GT.0) CALL UPTUSR(XSTOR2,XSTORV2,WV,IFLAG)
      IF (NCPVI.GT.0) CALL UPTCOP(XSTOR2,XSTORV2,WV,IFLAG)
      IF ((NPBGK.GT.0).AND.LBGKV)
     .   CALL UPTBGK(XSTOR2,XSTORV2,WV,NPBGK,IFLAG)

      WRITE(0,*) 'DEBUG: UPDMOL 3.00',IUPDTE
      IF (IUPDTE == 2) RETURN

      IF (.NOT.ALLOCATED(CNDYNM)) THEN
        WRITE(0,*) 'DEBUG: UPDMOL 4.00'
        ALLOCATE (CNDYNM(NMOL))
        DO IML=1,NMOLI
          CNDYNM(IML)=AMUA*RMASSM(IML)
        END DO
