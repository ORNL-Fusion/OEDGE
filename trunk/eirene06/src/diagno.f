...
C*DK OUTSIG
      ...
      REAL(DP) :: XPLEN(NCHEN), DUMFFS(NCHEN)
      REAL(DP) :: DUMFFD(NCHEN),DELENE(NCHEN),
     .            DELWL(NCHEN),DEL_HELP(NCHEN), VSPEC(NCHEN)
      REAL(DP) :: YMN2(1), YMX2(1), YMNLG2(1), YMXLG2(1), XMI, XMA
      REAL(DP) :: TIMA, DUMTIL, AH, TIMI, DEL1, DEL2, SUM,WLSHFT
      INTEGER :: ICURV, ICHORI, I, NCHNI, IDET, ISK, JSK, IERR
...
C  NEW HEADING ?
     ...
     .      'CX-DETECTOR SIGNALS, MAXW. NEUTRL. DISTR.'
        ELSEIF (NCHTAL(ICHORI).EQ.2) THEN
          CALL HEADNG('H ALPHA-DETECTOR SIGNALS: #/S/CM2/STERAD ',41)
          TXHEAD(1:41) = 
     .      'H ALPHA-DETECTOR SIGNALS: #/S/CM2/STERAD '
        ELSEIF (NCHTAL(ICHORI).EQ.3) THEN
          CALL HEADNG('SIDE ON SPECTRAL RADIANCE                ',41)
          TXHEAD(1:41) = 
...
C  photons
          IF (NSPSPZ(ICHORI).EQ.0) THEN
            TEXTS(1)=TEXTSS
          ENDIF
C  convert from energy to (inverted) wavelength scale
          SUM=0._DP
          WLSHFT=0._DP
     ...
     .            WLSHFT=HPCL/ESHIFT(ICHORI)*1.D7
          DO 370 I=1,NCHNI
            AH=FUFFER(ICHORI,I)
            DUMFFS(NCHNI-I+1)=MAX(1.E-30_DP,AH)
...
C  SET DOUBLE PREC. X SCALE FOR FORMATTED PRINTOUT
          DEL_HELP=XPLEN

          DUMFFS=DUMFFS*DELWL/DELENE
          DUMFFD=DUMFFD*DELWL/DELENE
C
          CALL MASRR2('WAVEL. ,RADIATIVE FLUX ',DEL_HELP,DUMFFD,NCHNI)
...
C  DUE TO DIRECT EMISSION FROMM SOURCE INTO LINE OF SIGHT,
