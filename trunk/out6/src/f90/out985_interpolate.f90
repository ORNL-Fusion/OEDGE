! This code was shamelessly ripped from http://users.bigpond.net.au/amiller/toms.html -SL, 13.01.06
! Changed from REAL to REAL*8 
! 
MODULE qshep2d
...
REAL*8, INTENT(IN)      :: x(:), y(:), f(:)
INTEGER, INTENT(OUT)  :: lcell(:,:), lnext(:), ier
REAL*8, INTENT(OUT)     :: xmin, ymin, dx, dy, rmax, rsq(n), a(:,:)
...
REAL*8 :: av, avsq, b(6,6), c, ddx, ddy, dmin, fk, rq, rs, rsmx,  &
        rsold, rws, s, sum, t, xk, xmn, yk, ymn

REAL*8, PARAMETER  :: dtol = 0.01, rtol = 1.0e-5, sf = 1.0
...
REAL*8, INTENT(IN)     :: px, py, x(:), y(:), f(:), xmin, ymin, dx, dy,  &
                        rmax, rsq(:), a(:,:)
REAL*8                 :: fn_val
...
! Local variables
REAL*8     :: delx, dely, ds, dxsq, dysq, rd, rds, rs, sw, swq, w, xp, yp
...
! Local variables
REAL*8     :: delx, dely, ds, dxsq, dysq, qk, qkx, qky, rd, rds, rs,  &
...
! slmod begin
REAL*8, INTENT(IN)         :: px, py, x(:), y(:), xmin, ymin, dx, dy
!
!REAL*8, INTENT(IN)         :: px, py, x(1), y(1), xmin, ymin, dx, dy
! slmod end
INTEGER, INTENT(OUT)     :: np
REAL*8, INTENT(OUT)        :: dsq
...
! Local variables
LOGICAL  :: first
REAL*8     :: delx, dely, r, rsmin, rsq, xp, yp
...
REAL*8, INTENT(IN)   :: xk, yk, fk, xi, yi, fi, s1, s2, r
REAL*8, INTENT(OUT)  :: row(6)
...
REAL*8  :: d, dx, dy, dxsq, dysq, w, w1, w2
...
REAL*8, INTENT(OUT)        :: xmin, ymin, dx, dy
INTEGER, INTENT(OUT)     :: ier
...
REAL*8     :: delx, dely, xmn, xmx, ymn, ymx
...
! slmod begin
  delx = (xmx-xmn) / DBLE(REAL(nnr))
  dely = (ymx-ymn) / DBLE(REAL(nnr))
!
...
REAL*8    :: a(5,36), dx, dy, eps, eq, eqx, eqy, f(36), p(10), px, py, q, q1, &
