...
c ICAL=3
      ...
      case default
         write(iunout,*) 'PHOTON MODULE (PH_INIT): ICAL=',ical,'N/A'
         call exit(1)
      end select
      RETURN
...
c  discrete distribution, all mass at e0=e00
         ...
         case(1)
            call dopplerprof(iipl,icell,dnd,drft)
            xx=e0-(e00+drft)
            fac = DOPPLER(xx,dnd)
         case(2)
            call naturalprof(gam)
            xx = e0-e00
            fac = LORENTZ(xx, gam)
         case(3)
            call voigtprof(iipl,icell,dnd,drft,gam)
            xx=(e0-(e00+drft))/dnd
            yy=gam*0.5_dp/dnd
            fac = DBLE(PH_FADDEEVA(xx,yy,dnd))
         case(4)
...
c           xx = l0-l00
c           hwvdw=fwhm
c           fac = ph_lorvdw(xx, fwhm, shift, dvdw, icell)
c           fac=fac*hpcl/(e0*e0)
c  use energy scale
            call  lorvdwprof(icell,fwhm,shift,dvdw,.true.)
            xx = E0-E00
            hwvdw=fwhm
            fac = ph_lorvdw(-xx, fwhm, -shift, dvdw, icell)
         case(5)
...
!normal zeeman delta or doppler
            gam=0._dp
            call zeeman_normalprof(icell,ctheta2,dbz)
            if(iptype == 6) then
               dnd=0._dp
               drft=0._dp
               fac = zm_profile(e0,ctheta2,dbz,gam,dnd,drft,e00,6)
            elseif (iptype == 7) then
               call dopplerprof(iipl,icell,dnd,drft)
               fac = zm_profile(e0,ctheta2,dbz,gam,dnd,drft,e00,7)
            endif
         case(8,9)
!normal zeeman lorentz or voigt
           ...
           if(iptype == 8) then
             dnd=0._dp
             drft=0._dp
             fac = zm_profile(e0,ctheta2,dbz,gam,dnd,drft,e00,8)
           elseif (iptype == 9) then
             call dopplerprof(iipl,icell,dnd,drft)
             fac = zm_profile(e0,ctheta2,dbz,gam,dnd,drft,e00,9)
           endif
         case(10,11)
...
c            npt=1
             ...
             fac = zm_stark_profile(dein(icell),T_e,T_p,T_g,Bf,
     .             ctheta2,v,e00,e0)
...
c            npt=1
             ...
             fac = zm_stark_doppler_profile(dein(icell),
     .             T_e,T_p,T_g,Bf,
...
c  --> divide by upper state density to make it a rate coefficient!
           ...
           res=res*fac
        case default
          write(iunout,*) 'PHOTON MODULE (PH_GETCOEFF): iid=',iid,
     .                    'not in use'
          res=0.
        end select ! iid
...
c     end select
      ...
      FUNCTION ZM_PROFILE(X, CTHETA2, DBZ, GAM,DND, DRFT,E00,IPROF)
     .         RESULT(RES)
...
! delta
        ...
        case(7)
! doppler
          val = doppler(x-(e00d+drft),dnd)
        case(8)
! lorentz
           val = lorentz(x-e00d,gam)
        case(9)
! voigt
          xx = (x-(e00d+drft))/dnd
          yy=gam*0.5_dp/dnd
          val = DBLE(PH_FADDEEVA(xx,yy,dnd))
...
c               all taken out.
       ...
       return
       end function zm_profile

      function zm_stark_profile(N,Te,Ti,T_g,B,
     .                          ctheta2,v,e00,e0)
     .                          result(res)
c    .                          npt,omega_min,omega_max)
...
c     integer,intent(inout)::npt
     ...
     .          e00,e0
      ...
      omega_Z=hbar*B/(2.*me)
      omega_D=.75*EI*v/c
      gamma=coll(N,Te,Ti,epsilon)+(hbar*A)/e
...
c  function Lorentz needs FWHM
     ...
     .  (Lorentz(omega-omega_D-2.*omega_plus,gam)
     .  +Lorentz(omega-omega_D-2.*omega_minus,gam)
     .  +C1*Lorentz(omega-omega_D-omega_Z+omega_minus-omega1,gam)
     .  +C2*Lorentz(omega-omega_D-omega_Z+omega_minus+omega1,gam)
     .  +C3*Lorentz(omega-omega_D+omega_Z+omega_plus-omega2,gam)
     .  +C4*Lorentz(omega-omega_D+omega_Z+omega_plus+omega2,gam))
     .  +(1.-ctheta2)*
     .  (C5*Lorentz(omega-omega_D+omega_plus-omega1,gam)
     .  +C6*Lorentz(omega-omega_D+omega_plus+omega1,gam)
     .  +C7*Lorentz(omega-omega_D+omega_minus-omega2,gam)
     .  +C8*Lorentz(omega-omega_D+omega_minus+omega2,gam))
...
c  was normalized to 4 for any fixed ctheta2.
        res=0.25* line_shape
c     end do   ! npt
      end function zm_stark_profile

      function zm_stark_doppler_profile(N,Te,Ti,T_g,B,
     .               ctheta2,v,dnd,drft,e00,e0)
     .               result(res)
c    .               npt,omega_min,omega_max
...
c        xx = (x-drft)/dnd
c        yy=gam*0.5_dp/dnd
c        val = DBLE(PH_FADDEEVA(xx,yy,dnd))
...
c  was normalized to 4 for any fixed ctheta2.
        res=0.25*line_shape
c     end do
      end function zm_stark_doppler_profile

!******************************************************************
      function coll(N,Te,Ti,epsilon)
...
!Physical and mathematical constants
      ...
      real(dp)::coll,N,Te,Ti,epsilon,v0,ve,rhoWi,rhoWe,
     .          lambda_Di,lambda_De,
...
!Integral from x=arg to x=infinity of exp(-x)/x
      ...
      if(arg.gt.1.0) then
        expint=exp(-arg)*(B4+arg*(B3+arg*(B2+arg*(B1+arg))))/
     .         (C4+arg*(C3+arg*(C2+arg*(C1+arg))))
        expint=expint/arg
      else
        expint=-log(arg)+A0+arg*(A1+arg*(A2+arg*(A3+arg*(A4+arg*A5))))
      end if
      end function expint
!****************************** END *******************************
...
!
      REAL(dp) FUNCTION PH_ENERGY(icell,kk,ipl2,VN,nldoppl) result(res)
      IMPLICIT NONE
...
c        call lorvdwprof(icell,fwhm,shift,dvdw,.false.)
c        l00=hpcl/E00
c        l0 = sam_lorentz(fwhm,l00+shift)
c        dl = sam_vdwqs(dvdw,1.E30_dp)
c        l0=l0+dl
...
c  default: sampling e0 and de=e00-e0 in energy (frequency) units
         call lorvdwprof(icell,fwhm,shift,dvdw,.true.)
         e0 = sam_lorentz(fwhm,e00+shift)
         de = sam_vdwqs(dvdw,e0)
         e0=e0-de
         res=e0
         nldoppl=.false.
         else
...
c  delta + normal zeeman triplet, no doppler
         ...
         dnd=0._dp
         drft=0._dp
         zep1 = sam_zeeman_normal(ctheta2,dbz,gam,dnd,drft,e00,0)
         res = zep1
         nldoppl=.false.
      case(7)
...
c  lorentz + normal zeeman triplet, no doppler
         ...
         dnd=0._dp
         drft=0._dp
         zep1 = sam_zeeman_normal(ctheta2,dbz,gam,dnd,drft,e00,2)
         res = zep1
         nldoppl=.false.
      case(9)
...
c  doppler broadening by emitting gas: hydrogen, deuterium, tritium
         ...
         ctheta2=velx*bxin(icell)+vely*byin(icell)+velz*bzin(icell)
         ctheta2=ctheta2*ctheta2
         zep1 = sam_zm_stark(dein(icell),T_e,T_p,T_g,
     .                       Bfin(icell),
     .                       ctheta2,E00,v)
         res = zep1
         nldoppl=.false.
...
c  doppler broadening by emitting gas: hydrogen, deuterium, tritium
     ...
     .                       ctheta2,E00,v)
      ...
      case default 
        write (iunout,*) 'profiletype in ph_energy ? exit called '
        call exit(1)
      end select
...
c        endif
         ...
         elseif(ityp2 == itypnew) then
            ...
            write(iunout,*) 'PHOTON MODULE (PH_POST_ENERGY):'
...
c        sample new freq, doppler shift due n=2 velocity
            ...
            case(2)
               call naturalprof(gam)
               zep1=sam_lorentz(gam,e00)
               ee = zep1
            case(3)
               call naturalprof(gam)
               een = e00*(1.-cvel/clight)
               zep1=sam_lorentz(gam,een)
               ee = zep1
            case(4)
c constants in wavelengths 
               call lorvdwprof(icell, fwhm,shift,dvdw,.false.)
               l00 = hpcl/e00
               l0  = sam_lorentz(fwhm,shift+l00)
               dl  = sam_vdwqs(dvdw,1.e30_dp)
               l0=l0+dl
               ee = hpcl/l0
            case(5)
c constants in wavelengths 
               ...
               ee  = ee*(1.-cvel/clight)
            case(6)
               call strkprof(icell,fwhm,shift)
               call naturalprof(gam)
               gam=gam+fwhm
...
c PROFILE PARAMETERS                                                 
      ...
      SUBROUTINE LORVDWPROF(icell,fwhm,shift,dvdw,lscale)
!
...
c dE / dlambda = -hc*lambda^-2 = -E^2/(hc)
      ...
      return
      end subroutine lorvdwprof

      subroutine voigtprof(ipl,icell,dnd,drft,gam)
...
!  return drft, the line shift due to the drift motion in the maxwellian
      ...
      call dopplerprof(ipl,icell,dnd,drft)
      call naturalprof(gam)
      return
      end subroutine voigtprof

      subroutine DOPPLERPROF(iipl,icell,dnd,drft)
...
!
     ...
     .       velz*vzin(iplv,icell)
      ...
      return
      end subroutine DOPPLERPROF

      subroutine NATURALPROF(gam)
...
c  convert to frequency (Hz), and then to energy, eV
      gam=gam*hplnk_bar
      return
      END subroutine NATURALPROF

      subroutine STRKPROF(icell, fwhm,shift)
!
...
!
      ...
      return
      end subroutine strkprof


      subroutine zeeman_normalprof(icell,ctheta2,dbz)
!
...
c
      ...
      return
      END subroutine zeeman_normalprof

      REAL(dp) FUNCTION LORENTZ(xx,gm) result(res)
...
!  special case: Cauchy distribution, for gm=2, shift=0
      ...
      REAL(dp) FUNCTION DOPPLER(xx,dnd) result(res)
!
...
!
      ...
      real(dp) function planck(e, t, b_nu, imode) result(res)
...
! rayleigh-jeans, e << kt
      ...
      complex(dp) function ph_faddeeva(x,y,dnd) result(cres)
      implicit none
      ...
      if(flag) then
         write(iunout,*) 'PHOTON MODULE (PH_FADDEEVA2):'
      ...
      real(dp) function ph_lorvdw(dx,fwhm,shift,dvdw,
     .                            icell) result(res)
...
c  next: part with imag. fadeeva function
      ...
      function sam_zeeman_normal(ctheta2,dbz,gam,dnd,drft,e00,iprof)
     .                                result(res)
!
...
c   build sampling distribution for sampling the component, given theta
      ...
      do ipol = -1,1
        ...
        if(r0 <= ssum) then
          ...
          case(0)
             res = e00d
          case(1)
             res = sam_doppler(dnd,drft,e00d)
          case(2)
             res = sam_lorentz(gam, e00d)
          case(3)
             res = sam_voigt(gam,dnd,drft,e00d)
          case default
      ...
      enddo
      return
      end function sam_zeeman_normal


      FUNCTION sam_zm_stark1(N,Te,Ti,T_g,B,
     .                       ctheta2,E00,v)
     .                       result(res)
...
c  prepare next interval
      ...
      end function sam_zm_stark1


      FUNCTION sam_zm_stark(N,Te,Ti,T_g,B,
     .                      ctheta2,E00,v)
     .                      result(res)
...
c  EIRENE function "Lorentz" needs FWHM, gamma is HWHM.
        gam=2._dp*gamma
        shift=x(i)
        res=sam_lorentz(gam,shift)

      end function sam_zm_stark

      REAL(dp) FUNCTION SAM_DOPPLER(dnd,drft,e00) result(res)
!
...
c     f2=v2*sqrt(-(ar+ar)/s)*sig
      res=f1+drft+e00
      END FUNCTION SAM_DOPPLER

      REAL(dp) FUNCTION SAM_VOIGT(alph,dnd,drft,e00) result(res)
!
...
*
      ...
      REAL(dp) FUNCTION SAM_LORENTZ(alph,shift) result(res)
...
!  x is sampled from Cauchy. Now transform to Lorentz(gam,shift)
      ...
      return
      END FUNCTION SAM_LORENTZ

      REAL(dp) FUNCTION SAM_VDWQS(dvdw,x0)
     .         result(res)
...
c  from convoluted profile.
      ...
      sig=1._dp/sqrt(beta)
      do
        xx=sam_doppler(sig,0._dp,0._dp)
        xxq=xx*xx
...
!pb     .         ph_lorvdw(-xx, fwhm, -shift, dvdw, icell)

            xx =-reaction%e0 
            phi_zero(iccnt,icell) = 
     .           ph_lorvdw(-xx, fwhm, -shift, dvdw, icell) 
...
! calculate function values at endpoints of rejection intervals 
     ...
     .                ph_lorvdw(-xxl, fwhm, -shift, dvdw, icell)

                  lrj = irj*l0right/reaction%nrjprt
                  errj = hpcl/lrj
                  xxr = errj-e00
                  phi_rj_right(iccnt,irj,icell) = 
     .                ph_lorvdw(-xxr, fwhm, -shift, dvdw, icell)
                end do
              else
      ...
      function sam_cutoff (ictoff,icell) result (res)
...
c  use rectangle for rejection
          ...
          do
            ...
            xx = xi1-E00
            phi_xi1 = ph_lorvdw(-xx, fwhm, -shift, dvdw, icell)
            if (xi2 <= phi_xi1) then
              res = xi1
...
!  use polygon for rejection
     ...
     .           xi1 + sint) / a
            ...
            xx = es-E00
            phi_xx = ph_lorvdw(-xx, fwhm, -shift, dvdw, icell)
            if (xi2 <= phi_xx) then
              res = es
c             write (iunout,*) ' left interval in sam_cutoff ',icount,
c    .                           res
