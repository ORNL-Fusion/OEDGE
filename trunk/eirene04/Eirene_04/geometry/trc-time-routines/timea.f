...
C
      ...
      IF (NLTRC) THEN
        CALL LEER(1)
        WRITE (6,*) 'TIMEA ,X,Y,Z,T ',X,Y,Z,T
        WRITE (6,*) '      VX,VY,VZ,V ',VX,VY,VZ,V
        IF (NLTRA) WRITE (6,*) 'MSURF,NNTCL ',MSURF,NNTCL
        IF (.NOT.NLTRA) WRITE (6,*) 'MSURF ',MSURF
      ENDIF
C
      TADD=0.
C
...
C  NO
     ...
     .            +(A3LM(J)+A6LM(J)*Z)*Z
        ...
        TMA=F+G
        TMI=F-G
        IF (NLTRC) WRITE (6,*) 'TIMEA, J,TMI,TMA ',J,TMI,TMA
...
C
40      TMA=-A2/A1
        IF (NLTRC) WRITE (6,*) 'TIMEA AT 40, J,TMA,A1,A2 ',J,TMA,A1,A2
...
50      IF (A2.EQ.0.) GOTO 100
        TMA=-A3/A2
        IF (NLTRC) WRITE (6,*) 'TIMEA AT 50, J,TMA,A2,A3 ',J,TMA,A2,A3
!PB        IF (TMA.LE.EPS12.OR.TMA.GT.TMIN) GOTO 100
        IF (TMA.LE.EPS12) GOTO 100
        IF (TMA.GT.TMIN) THEN
          IF (.NOT.NLPRCS(J)) THEN
...
70      TL=TMX+TADD
        ...
        SG=SIGN(1._DP,WR)
        IF (NLTRC) THEN
          WRITE (6,*) 'TIMEA, TL,XR,YR,ZR,NNR,MASURF,SG '
          WRITE (6,*)         TL,XR,YR,ZR,NNR,MASURF,SG
        ENDIF
C
...
C
      IF (NLTRC) WRITE (6,*) 'NOT RETURNED FROM TIMEA, OTHER LOOP '
