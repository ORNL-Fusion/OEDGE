...
C  FIND NPCELL IN STANDARD MESH, BLOCK NBLOCK
            IF (NLPOL) THEN
              ...
              ELSEIF (LEVGEO.EQ.3) THEN
                NPCELL=IPOLG
              ELSE
                WRITE (6,*) 'ERROR EXIT IN STDCOL, NLPOL ',LEVGEO
                CALL EXIT_OWN(1)
              ENDIF
            ELSE
              NPCELL=1
...
C  SWITCHING DONE
C
      IF (NLTRC.AND.ILIIN(MSURF).LE.3) CALL CHCTRC(X0,Y0,Z0,16,8)
C
C
...
C
999   CONTINUE
      WRITE (6,*) 'ERROR IN STDCOL, WRONG CELL SWITCHING '
      WRITE (6,*) 'NPANU, MSURF = ',NPANU,MSURF
      CALL EXIT_OWN(1)
      END
