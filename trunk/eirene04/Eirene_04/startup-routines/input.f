...
C
     ...
     .          SPCMN, SPCMX
      INTEGER :: IHELP(NLIMPS), IPRSF(12), NUMTAL(12)
     ...
     .           L, NZADD, INILGJ, INI, ICO, IS, NTLV, ID,
     ...
     .           ITLSOUT, NTLSOUT, IPLSTI, IPLSV
      INTEGER, EXTERNAL :: IDEZ
      LOGICAL :: LHELP(NLIMPS)
      LOGICAL :: LRPS3D, LRPSCN, NLSNGLV
...
!       NREAC_ADD = 0  ONLY FOR FIND-PARAM, NOT USED ANY FURTHER
     ...
     .                    NLERG,LIDENT,LHABER,LRCHRD
      ...
      ELSEIF (NFILEL.EQ.4) THEN
        WRITE (6,*) '       EIRENE READS PLASMA AND A&M DATA      '
        WRITE (6,*) '       FROM FILE FT13 '
        WRITE (6,*) '       SOURCE DISTRIBUTION IS OMITTED '
      ELSEIF (NFILEL.EQ.6) THEN
        WRITE (6,*) '       EIRENE SAVES PLASMA DATA, A&M DATA'
        WRITE (6,*) '       AND SOURCE DISTRIBUTION DATA'
        WRITE (6,*) '       ON FILE FT13 AT END OF RUN, I.E., AFTER'
        WRITE (6,*) '       LAST TIMESTEP OR ITERATION '
        WRITE (6,*) '       IN XDR FORMAT              '
...
C  READ ONE MORE LINE FOR NON-DEFAULT SPUTTER MODEL
          ...
          ELSEIF (ILSPT(NLJ).NE.0) THEN
            WRITE (6,*) 'WARNING: SPUTTERING AT NON DEF. SURFACE ',ISTS
            WRITE (6,*) 'BUT NO PARAMETERS RECYCS, RECYCC ARE READ '
            WRITE (6,*) 'DEFAULT MODEL: "NO SPUTTERING" IS USED. '
            WRITE (6,*) 'DO YOU REALLY WANT THIS?'
            ILSPT(NLJ)=0
          ENDIF
        ENDIF
C
...
C  READ ONE MORE LINE FOR NON-DEFAULT SPUTTER MODEL
          IF (ZEILE(1:1).NE.'*'.AND.ZEILE(1:9).NE.'TRANSFORM') THEN
            ...
            IREAD=0
          ELSEIF (ILSPT(NLJ).NE.0) THEN
            WRITE (6,*) 'WARNING: SPUTTERING FOR ADD. SURFACE ',NLJ
            WRITE (6,*) 'BUT NO PARAMETERS RECYCS, RECYCC ARE READ '
            WRITE (6,*) 'DEFAULT MODEL: "NO SPUTTERING" IS USED. '
            WRITE (6,*) 'DO YOU REALLY WANT THIS?'
            ILSPT(NLJ)=0
...
C
      ...
      CALL LEER(1)
!pb 08.05.2003
csw add n line, call PH_INIT(0) when NREACI defined
...
!  READ FILNAM
          ...
          IF (IEND-IANF+1 > 8) THEN
            WRITE (6,*) ' FILENAME FOR REACTION ',IR,' TOO LONG '
            CALL EXIT_OWN(1)
          END IF
...
!  READ H123
          ...
          IF (IEND-IANF+1 > 4) THEN
            WRITE (6,*) ' STRING H123 FOR REACTION ',IR,' TOO LONG '
            CALL EXIT_OWN(1)
          END IF
          H123='    '
...
!  READ REAC
          ...
          IF (IEND-IANF+1 > 50) THEN
            WRITE (6,*) ' REACTIONSTRING FOR REACTION ',IR,' TOO LONG '
            CALL EXIT_OWN(1)
          END IF
          REAC2=REPEAT(' ',50)
...
!  READ CRC
          ...
          IF (IEND-IANF+1 > 3) THEN
            WRITE (6,*) ' CRC-STRING FOR REACTION ',IR,' TOO LONG '
            CALL EXIT_OWN(1)
          END IF
...
C
      ...
      END IF
      IREAD=0
      WRITE (6,*) '*4D.   NEUTRAL PHOTONS SPECIES CARDS, NPHOTI SPECIES'
      READ (IUNIN,*) NPHOTI
...
!PB   TO BE WRITTEN
          CASE ('BOLTZMANN ')
            READ (IUNIN,'(I6,6X,2E12.4)') TDMPAR(IPLS)%TDM%ISP(1),
     .           TDMPAR(IPLS)%TDM%G_BOLTZ, TDMPAR(IPLS)%TDM%DELTAE
          CASE ('CORONA    ')
            READ (IUNIN,'(I6,1X,A6,1X,A4,A9,A3,E12.4)') 
     .           TDMPAR(IPLS)%TDM%ISP(1),TDMPAR(IPLS)%TDM%FNAME(1),
     .           TDMPAR(IPLS)%TDM%H2(1),TDMPAR(IPLS)%TDM%REACTION(1),
     .           TDMPAR(IPLS)%TDM%CR(1),TDMPAR(IPLS)%TDM%A_CORONA
            IF (INDEX(TDMPAR(IPLS)%TDM%H2(1),'H.2') == 0) THEN
              WRITE (6,*) ' WRONG REACTION SPECIFIED FOR CORONA MODEL '
              WRITE (6,*) ' ONLY H.2 REACTIONS ARE PERMITTED '
              WRITE (6,*) ' IPLS = ',IPLS
              CALL EXIT_OWN(1)
            END IF
          CASE ('COLRAD    ')
            DO I=1, TDMPAR(IPLS)%TDM%NRE
              READ (IUNIN,'(I6,1X,A6,1X,A4,A9,A3,E12.4)') 
     .             TDMPAR(IPLS)%TDM%ISP(I),TDMPAR(IPLS)%TDM%FNAME(I),
     .             TDMPAR(IPLS)%TDM%H2(I),TDMPAR(IPLS)%TDM%REACTION(I),
...
620   READ (IUNIN,6664) (DMLD(IMOL),IMOL=1,NMOLI)
     ...
     .                    REFCUR%EXPELR(1),REFCUR%EXPILR(1)
        do ispz=2,nspz
          REFCUR%JSRS(ispz) = REFCUR%JSRS(1)
          REFCUR%JSRC(ispz) = REFCUR%JSRC(1)
...
c  not a species card, hence: a sputer model card
     ...
     .                          REFCUR%STPRMR(1)
          ...
          if ((ideflt_spez > 0) .and. (ispz < 0)) then
            write (6,*) ' wrong card in species dep. reflection model'
            write (6,*) zeile
          end if
          READ (IUNIN,'(A72)') ZEILE
          IREAD=1
...
C
        IF (ideflt_sput.le.0.and.REFCUR%JLSPT.NE.0) THEN
          WRITE (6,*) 'WARNING: SPUTTERING FOR MODEL ',REFCUR%REFNAME
          WRITE (6,*) 'BUT NO PARAMETERS RECYCS, RECYCC ARE READ '
          WRITE (6,*) 'DEFAULT MODEL: "NO SPUTTERING" IS USED. '
          WRITE (6,*) 'DO YOU REALLY WANT THIS?'
          REFCUR%JLSPT=0
...
C
      ...
      IF (ZEILE(1:3) == '** ') THEN
        ...
        IF (NADSPC > 0) THEN
          ...
          IF ((NSTRAI > 1) .AND. (NSMSTRA > 0)) ALLOCATE(SMESTL(NADSPC))
        END IF
        DO J=1,NADSPC
          DO
            READ (IUNIN,'(A72)') ZEILE
          ...
          READ (IUNIN,'(6ES12.4)') SPCMN, SPCMX
          IF (ISPSRF > 0) THEN
            IF (ISPSRF > NLIMI) THEN
              WRITE (6,*) ' SURFACE INDEX FOR SPECTRUM OUT OF BOUNDS'
              WRITE (6,*) ' SPECTRUM NUMBER = ',J
              WRITE (6,*) ' SURFACE NUMBER = ',ISPSRF
              IERROR = IERROR + 1
            END IF
          ELSEIF (ISPSRF < 0) THEN
            ...
            ISPSRF = NLIM+ISPSRF
          ELSE
            WRITE (6,*) ' SURFACE INDEX 0 NOT FORSEEN '
            WRITE (6,*) ' SPECTRUM NUMBER = ',J
            IERROR = IERROR + 1
          END IF

          IF ((IPTYP < 0) .OR. (IPTYP > 4)) THEN
            WRITE (6,*) ' PARTICLE TYPE ',IPTYP,' NOT FORSEEN '
            WRITE (6,*) ' SPECTRUM NUMBER = ',J
            IERROR = IERROR + 1
          ELSE
     ...
     .          ((IPTYP == 4).AND.((IPSP < 0).OR.(IPSP > NPLSI)))) THEN
              WRITE (6,*) ' PARTICLE SPECIES INDEX OUT OF BOUNDS '
              WRITE (6,*) ' SPECTRUM NUMBER = ',J
              WRITE (6,*) ' SPECIES NUMBER = ',IPSP
              IERROR = IERROR + 1
            END IF
          END IF
...
C
      ...
      IF(NSURPR > NLIMPS) THEN
         WRITE (6,*) 'NSURPR > NLIMPS, EXIT.'
         WRITE (6,*) ' NSURPR, NLIMPS ', NSURPR, NLIMPS
         CALL EXIT_OWN(1)
      ENDIF
...
1130  CONTINUE
      ...
      IF ((ZEILE(1:1) .NE.'*') .AND. (SCAN(ZEILE,'FT') == 0)) THEN
        ...
        DO WHILE ((NTLVOUT > 0) .AND. (ITLVOUT < NTLVOUT))
          ...
          DO J=1,12
            IF ((NUMTAL(J) > 0) .AND. (NUMTAL(J) <= NTALV))
     .        LIVTALV(NUMTAL(J)) = .FALSE.
          END DO
          ITLVOUT=ITLVOUT+12
        END DO
        ...
        DO WHILE ((NTLSOUT > 0) .AND. (ITLSOUT < NTLSOUT))
          READ (IUNIN,6666) (NUMTAL(J),J=1,12)
          DO J=1,12
            IF ((NUMTAL(J) > 0) .AND. (NUMTAL(J) <= NTALS))
     .        LIVTALS(NUMTAL(J)) = .FALSE.
          END DO
          ITLSOUT=ITLSOUT+12
        END DO
      END IF
...
!  TEST IF CUTPLANE IS AXIS-PARALLEL       
     ...
     .      (CUTPLANE(3)*CUTPLANE(4) > 0._DP)) THEN
          WRITE (6,*) ' PLANE IS NOT PARALLEL TO ONE AXIS '
          WRITE (6,*) ' CUTTING OF TETRAHEDRONS ABANDONNED '
          WRITE (6,*) ' LRPSCUT SET TO .FALSE. '
          LRPSCUT = .FALSE.
        END IF
      END IF
      IPLANE=1
...
!pb 08.05.2003
csw call PH_INIT(1) *before* calling setamd(0)
...
!pb      CALL ALLOC_CSPEI
      CALL ALLOC_COMUSR(2)
      CALL ALLOC_CTEXT(2)
!pb      IF (ANY(INDPRO(1:6) == 6)) CALL ALLOC_BCKGRND
...
C
     ...
     .    (ANY(PLTL3D(1:NVOLPL)).AND.LEVGEO<=3))) CALL ALLOC_CGRPTL
      ...
      IF (NSIGVI > 0) THEN
        J=1
        DO WHILE (J <= NSIGVI)
          IF (.NOT.LIVTALV(IIH(J))) THEN
            WRITE (6,*) ' NO STATISTICS IS DONE FOR VOLUME TALLY NO. ',
     .                  IIH(J)
            WRITE (6,*) ' BECAUSE TALLY HAS BEEN SWITCHED OFF '
            IGH(J)=IGH(NSIGVI)
            IIH(J)=IIH(NSIGVI)
            NSIGVI=NSIGVI-1
          ELSE
      ...
      IF (NSIGSI > 0) THEN
        J=1
        DO WHILE (J <= NSIGSI)
          IF (.NOT.LIVTALS(IIHW(J))) THEN
            WRITE (6,*) ' NO STATISTICS IS DONE FOR SURFACE TALLY NO. ',
     .                  IIHW(J)
            WRITE (6,*) ' BECAUSE TALLY HAS BEEN SWITCHED OFF '
            IGHW(J)=IGHW(NSIGSI)
            IIHW(J)=IIHW(NSIGSI)
      ...
      IF (NSIGCI > 0) THEN
        J=1
        DO WHILE (J <= NSIGCI)
          IF (.NOT.LIVTALV(IIHC(1,J)) .OR. .NOT.LIVTALV(IIHC(2,J))) THEN
            WRITE (6,*) ' NO CORRELATION COEFFICIENT IS CALCULATED ',
     .                  ' BETWEEN TALLIES ',IIHC(1,J),' AND ',IIHC(2,J)
            WRITE (6,*) ' BECAUSE TALLIES HAVE BEEN SWITCHED OFF '
...
C  COMPUTE SOURCE DATA (OVERRULE SOME OF INPUT BLOCK 7)
      IF ((NMODE.NE.0.AND.IITER.LE.1) .OR. (IITER > NITER))  THEN
        ...
        ENDDO
      ENDIF
      IF ((NMODE == 0) .AND. (NSTEP > 0)) CALL ALLOC_CSTEP
C
      VOLTAL = 0.D0
      DO IN=1,NSBOX
        INT = NCLTAL(IN)
...
!  SWITCH OFF UNUSED TALLIES

      IF (ALL(NGENA(1:NATMI) == 0)) THEN
         LPGENA = .FALSE.
         LEGENA = .FALSE.
         LVGENA = .FALSE.
      END IF

      IF (ALL(NGENM(1:NMOLI) == 0)) THEN
         LPGENM = .FALSE.
         LEGENM = .FALSE.
         LVGENM = .FALSE.
      END IF

      IF (ALL(NGENI(1:NIONI) == 0)) THEN
         LPGENI = .FALSE.
         LEGENI = .FALSE.
         LVGENI = .FALSE.
      END IF

      IF (ALL(NGENPH(1:NPHOTI) == 0)) THEN
         LPGENPH = .FALSE.
         LEGENPH = .FALSE.
         LVGENPH = .FALSE.
      END IF
