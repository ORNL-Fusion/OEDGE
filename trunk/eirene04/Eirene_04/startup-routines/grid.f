...
!pb          WRITE (6,*) ' WRONG GRID OPTION SPECIFIED FOR FEM GRID '
!pb          CALL EXIT_OWN(1)
!pb        ENDIF
C
...
C
        ...
        IF (MAXVAL(INMTI(1:3,1:NTRII)) > NSTSI+NLIM) THEN
          WRITE (6,*) ' WRONG NUMBER OF REFLECTION MODEL SPECIFIED '
          WRITE (6,*) ' CHECK DEFINITION OF TRIANGLES ',
     .                'AND THEIR NEIGHBORS '
          CALL EXIT_OWN(1)
        END IF
C
     ...
     .                    nteck(itside(3,is),itet)
            ...
            IF (NTBAR(IS,ITET) < 0) THEN
              IF (SUM(NTBAR(1:4,ITET)) > -4) THEN
                WRITE (6,*) ' TETRAHEDRON WITH NEIGHBOR -1 DETECTED '
                WRITE (6,*) ' ITET = ',ITET,' ISIDE = ',IS
                WRITE (6,*) ' NTBAR(ITET) = ',NTBAR(1:4,ITET)
                IC=IC+1
              ELSE
                NTET_COLLAPS=NTET_COLLAPS+1
!pb                WRITE (6,*) ' COLLAPSED TETRAHEDRON ITET = ',ITET
                EXIT
              END IF
            END IF
          END DO
