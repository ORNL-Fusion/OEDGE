*DK HALFA
      SUBROUTINE HBETA (IST,IAD1,IAD2,IAD3,IAD4,IAD5,IADS)
c
C  SUBROUTINE FOR H-BETA EMISSIVITY. (BALMER SERIES)
C  TAKEN FROM EIRENE, SECTION DIAGNO, SUBR. SIGAL
C  THE H-BETA EMISSIVITY PROFILE (PHOTONS/S/CM**3) IS COMPUTED
...
c          WRITE(0,*) 'EXCLUDING CX CONTRIBUTION TO DBETA'
          WRITE(6,*) 'EXCLUDING CX CONTRIBUTION TO DBETA'
          REAC='2.0b    '
        ENDIF
c slmod end
CDR
...
c
        ...
        WRITE (6,*) 'ERROR IN HBETA: DATA FOR STRATUM ISTRA= ',IST
        WRITE (6,*) 'QUESTIONABLE, USE AT OWN RISK'
c
c        WRITE (6,*) 'ERROR IN HBETA: DATA FOR STRATUM ISTRA= ',IST
c        WRITE (6,*) 'ARE NOT AVAILABLE. H-BETA ABANDONNED'
c        RETURN
c slmod end
      ENDIF
C
...
c        IF (output)
c     .  WRITE(0,*) 'MARK: HBETA: NCELL= ',NCELL
c slmod end
        TE=TEIN(NCELL)
        DE=DEIN(NCELL)
        SIGADD1=0.
...
c         off:
     ...
     .        WRITE(0,*) 'NO Halpha CONTRIBUTION FROM RECOMBINATION'
          ELSE
            SIGADD2=SIGADD2+DP*FAC42
          ENDIF
c
c            SIGADD2=SIGADD2+DP*FAC42
c slmod end
205     CONTINUE
C
C  CHANNEL 3
...
1000  CONTINUE
      ...
      IF (IST.GT.0)
     .CALL MASBOX('SUBR. HBETA CALLED, FOR STRATUM NO. '//CISTRA)
      IF (IST.EQ.0)
     .CALL MASBOX('SUBR. HBETA CALLED, FOR SUM OVER STRATA')
      CALL LEER(1)
      WRITE (6,*) ' RADIATED POWER BY HBETA:',POWALF
