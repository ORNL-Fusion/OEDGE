...
;            print,val.v[j,0:2]
;            print,val.x[val.v[j,3]],val.y[val.v[j,3]]

            k = WHERE (j+1 EQ plot.show_tube, count)
            IF (count GT 0) THEN val_colour = plot.show_colour    [k[0]] ELSE  $
                                 val_colour = plot.annotate_colour[i   ]
