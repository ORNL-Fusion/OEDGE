...
CVKG TO FIX A BUG WITH GEOMETRY
                ...
                ISC2=0
                DO IS=1,3
                  IF(POINT_ON_INTERVAL(XTRIAN(NECKE(IS,IT)),
...
C
        ITRI=0
CVKG TO FIX GEOMETRY BUG
        IXNI=MAX(0,NINCT(ITARG,IPRT)) !VK
...
C  COUNT THE NUMBER OF CORNERS ON POLYGON NEXT TO THE TARGET
              ...
              DO IS=1,3
                IF(POINT_ON_INTERVAL(XTRIAN(NECKE(IS,IT)),
     f                           YTRIAN(NECKE(IS,IT)),
     f                           XPOL(IYTRI(IT),IXTRI(IT)+IXNI),
     f                           YPOL(IYTRI(IT),IXTRI(IT)+IXNI),
     f                           XPOL(IYTRI(IT)+1,IXTRI(IT)+IXNI),
     f                           YPOL(IYTRI(IT)+1,IXTRI(IT)+IXNI))) THEN
                  IF(ISC1.GT.0) THEN
                    ISC2=IS 
                  ELSE
                    ISC1=IS
...
C  THIS MEANS ONE SIDE OF TRIANGLE BELONGS TO TARGET
     ...
     .              ' STEP-FUNCTION '
                ...
                ENDIF
                NUMTRI(ITRI)=IT
                IF(ISC1.EQ.1.AND.ISC2.EQ.3) THEN
                  NUMSID(ITRI)=3
                ELSE
                  NUMSID(ITRI)=ISC1                  
                END IF
...
C  COUNT THE NUMBER OF CORNERS ON POLYGON NEXT TO THE TARGET
              ...
              DO IS=1,3
                IF(POINT_ON_INTERVAL(XTRIAN(NECKE(IS,IT)),
     f                           YTRIAN(NECKE(IS,IT)),
     f                           XPOL(IYTRI(IT)+IXNI,IXTRI(IT)),
     f                           YPOL(IYTRI(IT)+IXNI,IXTRI(IT)),
     f                           XPOL(IYTRI(IT)+IXNI,IXTRI(IT)+1),
     f                           YPOL(IYTRI(IT)+IXNI,IXTRI(IT)+1))) THEN
                 IF(ISC1.GT.0) THEN
                  ISC2=IS 
                 ELSE
                  ISC1=IS
...
C
       FUNCTION POINT_ON_INTERVAL(X,Y,X1,Y1,X2,Y2)
       LOGICAL :: POINT_ON_INTERVAL
       REAL(DP),INTENT(IN) :: X,Y,X1,Y1,X2,Y2
       REAL(DP) :: A,B,C,D,L
      
       POINT_ON_INTERVAL=.FALSE.
       
       IF(X1.LT.X2.AND.
     f    (X.GT.X2+EPS5.OR.X.LT.X1-EPS5)) RETURN
       IF(X1.GT.X2.AND.
     f    (X.GT.X1+EPS5.OR.X.LT.X2-EPS5)) RETURN
       IF(Y1.LT.Y2.AND.
     f    (Y.GT.Y2+EPS5.OR.Y.LT.Y1-EPS5)) RETURN
       IF(Y1.GT.Y2.AND.
     f    (Y.GT.Y1+EPS5.OR.Y.LT.Y2-EPS5)) RETURN
       ...
       L=A*A+B*B 
       IF(L.LT.EPS5) THEN
        WRITE(6,*) "WARNING FROM  POINT_ON_INTERVAL"
        WRITE(6,*) "THE LENGTH OF THE INTERVAL IS TOO SMALL"
        WRITE(6,*) "L,X1,Y1,X2,Y2 ",L,X1,Y1,X2,Y2
        RETURN
       END IF
       IF(D.LT.1.D-8*L) POINT_ON_INTERVAL=.TRUE.   

       RETURN

       END FUNCTION POINT_ON_INTERVAL
