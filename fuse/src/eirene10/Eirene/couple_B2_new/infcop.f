...
C
      ...
      LOGICAL, SAVE :: LSHORT, LSTOP, LTEST
...
7000    CONTINUE
C
        IF (.NOT.LSHORT) CALL SAVE_TALLIES(ISTRAI)
C
...
!pb                SMOCL=(COPV(IADD+IPLS,INC)+CPPV(IADD+IPLS,INC))*
     ...
     .                CPPV(IPLS,INC))*VOLTAL(INC)*1.D-5*SIGNUM*FLX_EIR
...
!pb        NLSRON(ISTRAI) = .NOT.LTEST 
        NLSRON(ISTRAI) = (.NOT.LTEST) .OR. (XMCP(ISTRAI) <= 2)
C
        IF (.NOT.LSHORT) THEN
C
          DO 7560 IX=0,NDXA+1
...
C
     ...
     .        SUM(RESSMO(0,1:NFLA)))
        ...
        CALL MASRR1 (' RESSNI    ',RESSNI(0,1:NFLA),NFLA,5)
        WRITE (iunout,*) ' RESSMO-CONTRIBUTIONS BY DIFFERENT SPECIES '
        CALL MASRR1 (' RESSMO    ',RESSMO(0,1:NFLA),NFLA,5)
      ENDIF
C
      CALL LEER (1)
C
