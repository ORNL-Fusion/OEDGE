...
212         CONTINUE
     ...
     .          ' TRIANGLE OR TETRAHEDRON GRID SWITCHED ON '
            ...
            IF (NLFEM) CALL READ_TRIANG (CASENAME(1:I2))
            IF (NLTET) CALL READ_TETRA (CASENAME(1:I2))
...
C
     ...
     .                  .FALSE.)
        ...
        IEND = IEND + 7 + ITOK
        CALL READ_TOKEN(ZEILE(IEND+1:),' ',CADAPT,ITOK,IER,
     .                  .FALSE.)
...
!  READ REAC
        IF (INDEX(FILNAM,'CONST') == 0) THEN
          CALL READ_TOKEN(ZEILE(IEND:),' ',REAC2,ITOK,IER,.FALSE.)
          IEND = IEND + ITOK
          IF (IER > 0) THEN
            WRITE (iunout,*) 
...
!  CHECK LINE FOR FT-FLAG (IFTFLG)
          ...
          IF (ITOK /= 0) THEN
            ...
            IEND = IEND+ITOK+2
            CALL READ_TOKEN(ZEILE(IEND:),' ',CHR,ITOK,IER,.FALSE.)
            IEND = IEND + ITOK
          END IF
        END IF
!  READ CRC
        CALL READ_TOKEN(ZEILE(IEND:),' ',CRC,ITOK,IER,.FALSE.)
        IEND = IEND + ITOK
        IF (IER > 0) THEN
...
!  READ MT
        ...
        END IF
!  READ DPP
        CALL READ_TOKEN(ZEILE(IEND:),' ',CHR,ITOK,IER,.TRUE.)
        IEND = IEND + ITOK
        READ (CHR,'(E12.4)') DPP
        IF (IER > 0) THEN
...
c  cell volume -profile
     ...
     ,        (INDEX(ZEILE,'6.') > 0)) EXIT
      ...
      IF (LHYDDEF) CALL SETUP_HYDKIN_REACTIONS(HYDKIN_DEFAULT, CADAPT)
C
