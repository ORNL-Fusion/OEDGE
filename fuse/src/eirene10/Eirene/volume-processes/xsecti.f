...
C
     ...
     .          EHEAVY, EELEC, EBULK, COU, RATE_COEFF, ERATE
...
7000      ACCMAS=0.D0
          ...
          IF (NSTORDR >= NRAD) THEN
            DO 73 J=1,NSBOX
              COU = RATE_COEFF(-8,TEINL(J),0._DP,.TRUE.,0,ERATE)
...
C  SECOND PROCESS
          ...
          IF (NSTORDR >= NRAD) THEN
            DO 71 J=1,NSBOX
              COU = RATE_COEFF(-9,TEINL(J),0._DP,.TRUE.,0,ERATE)
...
C  THIRD PROCESS
          ...
          IF (NSTORDR >= NRAD) THEN
            DO 72 J=1,NSBOX
              COU = RATE_COEFF(-10,TEINL(J),0._DP,.TRUE.,0,ERATE)
              TABDS1(IREI,J)=COU*DEIN(J)
72          CONTINUE
