...
C
     ...
     .          RMASS, EBULK, EHEAVY, COU, RATE_COEFF, ERATE
...
C  FIRST PROCESS
          ...
          IF (NSTORDR >= NRAD) THEN
            DO 70 J=1,NSBOX
              COU = RATE_COEFF(-5,TEINL(J),0._DP,.TRUE.,0,ERATE)
...
73        ACCMAS=0.D0
          ...
          IF (NSTORDR >= NRAD) THEN
            DO 71 J=1,NSBOX
              COU = RATE_COEFF(-6,TEINL(J),0._DP,.TRUE.,0,ERATE)
...
C  THIRD PROCESS
          ...
          IF (NSTORDR >= NRAD) THEN
            DO 72 J=1,NSBOX
              COU = RATE_COEFF(-7,TEINL(J),0._DP,.TRUE.,0,ERATE)
              TABDS1(IREI,J)=COU*DEIN(J)
72          CONTINUE
